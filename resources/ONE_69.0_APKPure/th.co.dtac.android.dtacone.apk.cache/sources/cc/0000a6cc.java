...
public class ExternalAccountMetricsHandler implements Serializable {
    ...
    public String getExternalAccountMetricsHeader() {
        return String.format("%s %s %s/%s %s/%s %s/%s", MetricsUtils.m41627c(), BYOID_METRICS_SECTION, "source", this.credentials.getCredentialSourceType(), IMPERSONATION_KEY, Boolean.valueOf(this.saImpersonation), CONFIG_LIFETIME_KEY, Boolean.valueOf(this.configLifetime));
    }
}