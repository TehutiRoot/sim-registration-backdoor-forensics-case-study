...
public final class TaskLoggerKt {
    ...
    public static final <T> T logElapsed(@NotNull Task task, @NotNull TaskQueue queue, @NotNull Function0<? extends T> block) {
        ...
        try {
            ...
            if (isLoggable) {
                log(task, queue, Intrinsics.stringPlus("finished run in ", formatDuration(queue.getTaskRunner$okhttp().getBackend().nanoTime() - j)));
            }
            ...
            InlineMarker.finallyStart(1);
            if (isLoggable) {
                log(task, queue, Intrinsics.stringPlus("failed a run in ", formatDuration(queue.getTaskRunner$okhttp().getBackend().nanoTime() - j)));
            }
            InlineMarker.finallyEnd(1);
            throw th2;
        }
