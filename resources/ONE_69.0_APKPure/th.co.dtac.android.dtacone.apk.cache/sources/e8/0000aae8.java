...
public final class SparseImmutableTable<R, C, V> extends RegularImmutableTable<R, C, V> {
    static final ImmutableTable<Object, Object, Object> EMPTY = new SparseImmutableTable(ImmutableList.m40987of(), ImmutableSet.m40920of(), ImmutableSet.m40920of());
    ...
    public SparseImmutableTable(ImmutableList<Table.Cell<R, C, V>> immutableList, ImmutableSet<R> immutableSet, ImmutableSet<C> immutableSet2) {
        ImmutableMap m40676u = Maps.m40676u(immutableSet);
        ...
        for (int i = 0; i < immutableList.size(); i++) {
            ...
            Integer num = (Integer) m40676u.get(rowKey);
    ...
    public Object writeReplace() {
        ImmutableMap m40676u = Maps.m40676u(columnKeySet());
        int[] iArr = new int[cellSet().size()];
        UnmodifiableIterator<Table.Cell<R, C, V>> it = cellSet().iterator();
        int i = 0;
        while (it.hasNext()) {
            Integer num = (Integer) m40676u.get(it.next().getColumnKey());
            Objects.requireNonNull(num);
            iArr[i] = num.intValue();
            i++;
        }
