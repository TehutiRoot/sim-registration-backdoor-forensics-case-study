...
public class AbsSpinnerBindingAdapter {
    ...
    public static <T> void setEntries(AbsSpinner absSpinner, List<T> list) {
        if (list != null) {
            SpinnerAdapter adapter = absSpinner.getAdapter();
            if (adapter instanceof C20421iK0) {
                ((C20421iK0) adapter).m30986b(list);
                return;
            } else {
                absSpinner.setAdapter((SpinnerAdapter) new C20421iK0(absSpinner.getContext(), list, 17367048, 17367049, 0));
                return;
            }
        }
        absSpinner.setAdapter((SpinnerAdapter) null);
