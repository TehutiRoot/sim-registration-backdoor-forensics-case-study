...
public class IESEngine {
    ...
    private byte[] decryptBlock(byte[] bArr, int i, int i2) throws InvalidCipherTextException {
        ...
        byte[] bArr3;
        int processBytes;
        if (i2 >= this.f75096V.length + this.mac.getMacSize()) {
            if (this.cipher == null) {
                int length = (i2 - this.f75096V.length) - this.mac.getMacSize();
                ...
                for (int i4 = 0; i4 != length; i4++) {
                    bArr3[i4] = (byte) (bArr[(this.f75096V.length + i) + i4] ^ bArr4[i4]);
                }
                processBytes = 0;
            } else {
    ...
    private byte[] encryptBlock(byte[] bArr, int i, int i2) throws InvalidCipherTextException {
        ...
        if (this.cipher == null) {
            ...
            if (this.f75095IV != null) {
                bufferedBlockCipher = this.cipher;
                keyParameter = new ParametersWithIV(new KeyParameter(bArr6), this.f75095IV);
            } else {
                bufferedBlockCipher = this.cipher;
        ...
        byte[] bArr10 = this.f75096V;
        byte[] bArr11 = new byte[bArr10.length + i2 + macSize];
        System.arraycopy(bArr10, 0, bArr11, 0, bArr10.length);
        System.arraycopy(bArr2, 0, bArr11, this.f75096V.length, i2);
        System.arraycopy(bArr9, 0, bArr11, this.f75096V.length + i2, macSize);
        return bArr11;
    }
    ...
    private void extractParams(CipherParameters cipherParameters) {
        if (cipherParameters instanceof ParametersWithIV) {
            ...
            this.f75095IV = parametersWithIV.getIV();
    ...
    public byte[] processBlock(byte[] bArr, int i, int i2) throws InvalidCipherTextException {
        if (this.forEncryption) {
            ...
            if (ephemeralKeyPairGenerator != null) {
                ...
                this.f75096V = generate.getEncodedPublicKey();
            }
        } else if (this.keyParser != null) {
            ByteArrayInputStream byteArrayInputStream = new ByteArrayInputStream(bArr, i, i2);
            try {
                this.pubParam = this.keyParser.readKey(byteArrayInputStream);
                this.f75096V = Arrays.copyOfRange(bArr, i, (i2 - byteArrayInputStream.available()) + i);
            } catch (IOException e) {
