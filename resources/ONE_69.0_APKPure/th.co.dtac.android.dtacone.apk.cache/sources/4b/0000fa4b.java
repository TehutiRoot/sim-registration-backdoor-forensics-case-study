...
import org.bouncycastle.jcajce.p037io.OutputStreamFactory;
...
public abstract class X509CertificateImpl extends X509Certificate implements BCX509Certificate {
    ...
    private void checkSignature(PublicKey publicKey, Signature signature, ASN1Encodable aSN1Encodable, byte[] bArr) throws CertificateException, NoSuchAlgorithmException, SignatureException, InvalidKeyException {
        if (!isAlgIdEqual(this.f75361c.getSignatureAlgorithm(), this.f75361c.getTBSCertificate().getSignature())) {
        ...
        try {
            ...
            this.f75361c.getTBSCertificate().encodeTo(bufferedOutputStream, ASN1Encoding.DER);
    ...
    private void doVerify(PublicKey publicKey, SignatureCreator signatureCreator) throws CertificateException, NoSuchAlgorithmException, InvalidKeyException, SignatureException, NoSuchProviderException {
        ...
        int i = 0;
        if (z && X509SignatureUtil.isCompositeAlgorithm(this.f75361c.getSignatureAlgorithm())) {
            List<PublicKey> publicKeys = ((CompositePublicKey) publicKey).getPublicKeys();
            ASN1Sequence aSN1Sequence = ASN1Sequence.getInstance(this.f75361c.getSignatureAlgorithm().getParameters());
            ASN1Sequence aSN1Sequence2 = ASN1Sequence.getInstance(DERBitString.getInstance(this.f75361c.getSignature()).getBytes());
            boolean z2 = false;
        ...
        } else if (!X509SignatureUtil.isCompositeAlgorithm(this.f75361c.getSignatureAlgorithm())) {
            Signature createSignature = signatureCreator.createSignature(X509SignatureUtil.getSignatureName(this.f75361c.getSignatureAlgorithm()));
            if (!z) {
                checkSignature(publicKey, createSignature, this.f75361c.getSignatureAlgorithm().getParameters(), getSignature());
                return;
            }
            List<PublicKey> publicKeys2 = ((CompositePublicKey) publicKey).getPublicKeys();
            while (i != publicKeys2.size()) {
                try {
                    checkSignature(publicKeys2.get(i), createSignature, this.f75361c.getSignatureAlgorithm().getParameters(), getSignature());
                    return;
            ...
            ASN1Sequence aSN1Sequence3 = ASN1Sequence.getInstance(this.f75361c.getSignatureAlgorithm().getParameters());
            ASN1Sequence aSN1Sequence4 = ASN1Sequence.getInstance(DERBitString.getInstance(this.f75361c.getSignature()).getBytes());
            boolean z3 = false;
    ...
    public Set getCriticalExtensionOIDs() {
        if (getVersion() == 3) {
            ...
            Extensions extensions = this.f75361c.getTBSCertificate().getExtensions();
            if (extensions != null) {
    ...
    public byte[] getEncoded() throws CertificateEncodingException {
        try {
            return this.f75361c.getEncoded(ASN1Encoding.DER);
        } catch (IOException e) {
            throw new CertificateEncodingException(e.toString());
        }
    }
    ...
    public List getExtendedKeyUsage() throws CertificateParsingException {
        byte[] extensionOctets = getExtensionOctets(this.f75361c, "2.5.29.37");
        if (extensionOctets == null) {
            return null;
        }
        try {
    ...
    public Collection getIssuerAlternativeNames() throws CertificateParsingException {
        return getAlternativeNames(this.f75361c, Extension.issuerAlternativeName.getId());
    }

    @Override // java.security.cert.X509Certificate
    public Principal getIssuerDN() {
        return new X509Principal(this.f75361c.getIssuer());
    }

    @Override // java.security.cert.X509Certificate
    public boolean[] getIssuerUniqueID() {
        DERBitString issuerUniqueId = this.f75361c.getTBSCertificate().getIssuerUniqueId();
    ...
    public X500Name getIssuerX500Name() {
        return this.f75361c.getIssuer();
    }

    @Override // java.security.cert.X509Certificate
    public X500Principal getIssuerX500Principal() {
        try {
            return new X500Principal(this.f75361c.getIssuer().getEncoded(ASN1Encoding.DER));
    ...
    public Date getNotAfter() {
        return this.f75361c.getEndDate().getDate();
    }

    @Override // java.security.cert.X509Certificate
    public Date getNotBefore() {
        return this.f75361c.getStartDate().getDate();
    }

    @Override // java.security.cert.Certificate
    public PublicKey getPublicKey() {
        try {
            return BouncyCastleProvider.getPublicKey(this.f75361c.getSubjectPublicKeyInfo());
        } catch (IOException unused) {
            return null;
        }
    }
    ...
    public BigInteger getSerialNumber() {
        return this.f75361c.getSerialNumber().getValue();
    }
    ...
    public String getSigAlgOID() {
        return this.f75361c.getSignatureAlgorithm().getAlgorithm().getId();
    }
    ...
    public byte[] getSignature() {
        return this.f75361c.getSignature().getOctets();
    }

    @Override // java.security.cert.X509Certificate
    public Collection getSubjectAlternativeNames() throws CertificateParsingException {
        return getAlternativeNames(this.f75361c, Extension.subjectAlternativeName.getId());
    }

    @Override // java.security.cert.X509Certificate
    public Principal getSubjectDN() {
        return new X509Principal(this.f75361c.getSubject());
    }

    @Override // java.security.cert.X509Certificate
    public boolean[] getSubjectUniqueID() {
        DERBitString subjectUniqueId = this.f75361c.getTBSCertificate().getSubjectUniqueId();
    ...
    public X500Name getSubjectX500Name() {
        return this.f75361c.getSubject();
    }

    @Override // java.security.cert.X509Certificate
    public X500Principal getSubjectX500Principal() {
        try {
            return new X500Principal(this.f75361c.getSubject().getEncoded(ASN1Encoding.DER));
        } catch (IOException unused) {
    ...
    public byte[] getTBSCertificate() throws CertificateEncodingException {
        try {
            return this.f75361c.getTBSCertificate().getEncoded(ASN1Encoding.DER);
        } catch (IOException e) {
            throw new CertificateEncodingException(e.toString());
        }
    }
    ...
    public TBSCertificate getTBSCertificateNative() {
        return this.f75361c.getTBSCertificate();
    }

    @Override // java.security.cert.X509Certificate
    public int getVersion() {
        return this.f75361c.getVersionNumber();
    }

    @Override // java.security.cert.X509Extension
    public boolean hasUnsupportedCriticalExtension() {
        Extensions extensions;
        if (getVersion() != 3 || (extensions = this.f75361c.getTBSCertificate().getExtensions()) == null) {
            return false;
        }
        Enumeration oids = extensions.oids();
        while (oids.hasMoreElements()) {
    ...
    public void checkValidity(Date date) throws CertificateExpiredException, CertificateNotYetValidException {
        if (date.getTime() > getNotAfter().getTime()) {
            throw new CertificateExpiredException("certificate expired on " + this.f75361c.getEndDate().getTime());
        } else if (date.getTime() >= getNotBefore().getTime()) {
        } else {
            throw new CertificateNotYetValidException("certificate not valid till " + this.f75361c.getStartDate().getTime());
        }
    }

    @Override // java.security.cert.X509Extension
    public byte[] getExtensionValue(String str) {
        ASN1OctetString extensionValue = getExtensionValue(this.f75361c, str);
        if (extensionValue != null) {
            try {
