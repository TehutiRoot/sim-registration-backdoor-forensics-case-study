...
public final class _PathKt {
    ...
    /* renamed from: c */
    public static final boolean m25979c(Path path) {
        if (!path.getBytes$okio().endsWith(f73004e) || (path.getBytes$okio().size() != 2 && !path.getBytes$okio().rangeEquals(path.getBytes$okio().size() - 3, f73000a, 0, 1) && !path.getBytes$okio().rangeEquals(path.getBytes$okio().size() - 3, f73001b, 0, 1))) {
            return false;
        }
        return true;
    }
    ...
    public static final boolean commonIsRoot(@NotNull Path path) {
        Intrinsics.checkNotNullParameter(path, "<this>");
        if (m25978d(path) == path.getBytes$okio().size()) {
            return true;
        }
        return false;
    }
    ...
    public static final ByteString commonNameBytes(@NotNull Path path) {
        Intrinsics.checkNotNullParameter(path, "<this>");
        int m25981a = m25981a(path);
        if (m25981a != -1) {
            return ByteString.substring$default(path.getBytes$okio(), m25981a + 1, 0, 2, null);
        }
    ...
    public static final Path commonParent(@NotNull Path path) {
        Intrinsics.checkNotNullParameter(path, "<this>");
        if (!Intrinsics.areEqual(path.getBytes$okio(), f73003d) && !Intrinsics.areEqual(path.getBytes$okio(), f73000a) && !Intrinsics.areEqual(path.getBytes$okio(), f73001b) && !m25979c(path)) {
            int m25981a = m25981a(path);
            if (m25981a == 2 && path.volumeLetter() != null) {
                if (path.getBytes$okio().size() == 3) {
                    return null;
                }
                return new Path(ByteString.substring$default(path.getBytes$okio(), 0, 3, 1, null));
            } else if (m25981a == 1 && path.getBytes$okio().startsWith(f73001b)) {
                return null;
            } else {
                if (m25981a == -1 && path.volumeLetter() != null) {
                    if (path.getBytes$okio().size() == 2) {
                        return null;
                    }
                    return new Path(ByteString.substring$default(path.getBytes$okio(), 0, 2, 1, null));
                } else if (m25981a == -1) {
                    return new Path(f73003d);
                } else {
                    if (m25981a == 0) {
                        return new Path(ByteString.substring$default(path.getBytes$okio(), 0, 1, 1, null));
                    }
                    return new Path(ByteString.substring$default(path.getBytes$okio(), 0, m25981a, 1, null));
                }
            }
        }
        return null;
    ...
    public static final Path commonRelativeTo(@NotNull Path path, @NotNull Path other) {
        ...
        if (Intrinsics.areEqual(path.getRoot(), other.getRoot())) {
            ...
            if (segmentsBytes2.subList(i, segmentsBytes2.size()).indexOf(f73004e) == -1) {
                Buffer buffer = new Buffer();
                ByteString m25980b = m25980b(other);
                if (m25980b == null && (m25980b = m25980b(path)) == null) {
                    m25980b = m25975g(Path.DIRECTORY_SEPARATOR);
                }
                int size = segmentsBytes2.size();
                for (int i2 = i; i2 < size; i2++) {
                    buffer.write(f73004e);
                    buffer.write(m25980b);
                }
    ...
    public static final Path commonRoot(@NotNull Path path) {
        Intrinsics.checkNotNullParameter(path, "<this>");
        int m25978d = m25978d(path);
        if (m25978d == -1) {
            return null;
        }
        return new Path(path.getBytes$okio().substring(0, m25978d));
    }
    ...
    public static final List<String> commonSegments(@NotNull Path path) {
        ...
        if (m25978d == -1) {
            m25978d = 0;
        } else if (m25978d < path.getBytes$okio().size() && path.getBytes$okio().getByte(m25978d) == 92) {
            m25978d++;
        }
        int size = path.getBytes$okio().size();
        int i = m25978d;
        while (m25978d < size) {
            if (path.getBytes$okio().getByte(m25978d) == 47 || path.getBytes$okio().getByte(m25978d) == 92) {
                arrayList.add(path.getBytes$okio().substring(i, m25978d));
                i = m25978d + 1;
            }
            m25978d++;
        }
    ...
    public static final Character commonVolumeLetter(@NotNull Path path) {
        Intrinsics.checkNotNullParameter(path, "<this>");
        if (ByteString.indexOf$default(path.getBytes$okio(), f73000a, 0, 2, (Object) null) != -1 || path.getBytes$okio().size() < 2 || path.getBytes$okio().getByte(1) != 58) {
            return null;
        }
    ...
    /* renamed from: d */
    public static final int m25978d(Path path) {
        ...
        if (path.getBytes$okio().getByte(0) == 92) {
            ...
            int indexOf = path.getBytes$okio().indexOf(f73001b, 2);
            if (indexOf == -1) {
    ...
    /* renamed from: e */
    public static final boolean m25977e(Buffer buffer, ByteString byteString) {
        if (!Intrinsics.areEqual(byteString, f73001b) || buffer.size() < 2 || buffer.getByte(1L) != 58) {
            return false;
        }
    ...
    /* renamed from: f */
    public static final ByteString m25976f(byte b) {
        if (b != 47) {
            if (b == 92) {
                return f73001b;
            }
    ...
    /* renamed from: g */
    public static final ByteString m25975g(String str) {
    ...
    public static final Path toPath(@NotNull Buffer buffer, boolean z) {
        ...
        int i = 0;
        while (true) {
            if (!buffer.rangeEquals(0L, f73000a)) {
                byteString = f73001b;
        ...
        if (z2) {
            ...
            long indexOfElement = buffer.indexOfElement(f73002c);
            if (byteString2 == null) {
                if (indexOfElement == -1) {
                    byteString2 = m25975g(Path.DIRECTORY_SEPARATOR);
                } else {
                    byteString2 = m25976f(buffer.getByte(indexOfElement));
                }
            }
            if (m25977e(buffer, byteString2)) {
                if (indexOfElement == 2) {
        ...
        while (!buffer.exhausted()) {
            long indexOfElement2 = buffer.indexOfElement(f73002c);
            ...
            ByteString byteString3 = f73004e;
                ...
                if (!z3 || !arrayList.isEmpty()) {
                    if (z && (z3 || (!arrayList.isEmpty() && !Intrinsics.areEqual(CollectionsKt___CollectionsKt.last((List<? extends Object>) arrayList), byteString3)))) {
                        if (!z2 || arrayList.size() != 1) {
                            AbstractC10396hs.removeLastOrNull(arrayList);
                        }
                    } else {
                        arrayList.add(readByteString);
                    }
                }
            } else if (!Intrinsics.areEqual(readByteString, f73003d) && !Intrinsics.areEqual(readByteString, ByteString.EMPTY)) {
                arrayList.add(readByteString);
            }
        }
    ...
    public static final Path commonResolve(@NotNull Path path, @NotNull Path child, boolean z) {
        ...
        ByteString m25980b = m25980b(path);
        if (m25980b == null && (m25980b = m25980b(child)) == null) {
            m25980b = m25975g(Path.DIRECTORY_SEPARATOR);
        }
        Buffer buffer = new Buffer();
