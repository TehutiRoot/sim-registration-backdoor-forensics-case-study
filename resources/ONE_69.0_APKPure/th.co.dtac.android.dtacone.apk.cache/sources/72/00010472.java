...
public abstract class BasePeriod extends AbstractPeriod implements ReadablePeriod, Serializable {
    private static final ReadablePeriod DUMMY_PERIOD = new C12976a();
    ...
    /* loaded from: classes5.dex */
    public static class C12976a extends AbstractPeriod {
    ...
    private void checkAndUpdate(DurationFieldType durationFieldType, int[] iArr, int i) {
        ...
        if (indexOf == -1) {
            if (i != 0) {
                throw new IllegalArgumentException("Period does not support field '" + durationFieldType.getName() + OperatorName.SHOW_TEXT_LINE);
            }
            return;
        }
        iArr[indexOf] = i;
    ...
    public void addFieldInto(int[] iArr, DurationFieldType durationFieldType, int i) {
        ...
        if (indexOf == -1) {
            if (i != 0 || durationFieldType == null) {
                throw new IllegalArgumentException("Period does not support field '" + durationFieldType + OperatorName.SHOW_TEXT_LINE);
            }
            return;
        }
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (value != 0) {
                ...
                if (indexOf != -1) {
                    ...
                    throw new IllegalArgumentException("Period does not support field '" + fieldType.getName() + OperatorName.SHOW_TEXT_LINE);
                }
            }
        }
        return iArr;
