...
public final class CharsetReader {
    ...
    public CharsetReader(@NotNull InputStream inputStream, @NotNull Charset charset) {
        ...
        this.f71403a = inputStream;
        this.f71404b = charset;
        ...
        this.f71405c = onUnmappableCharacter;
    ...
    /* renamed from: a */
    public final int m26937a(char[] cArr, int i, int i2) {
        ...
        boolean z = false;
        while (true) {
            CoderResult decode = this.f71405c.decode(this.f71406d, wrap, z);
            if (decode.isUnderflow()) {
                if (z || !wrap.hasRemaining()) {
                    break;
                } else if (m26936b() < 0) {
                    if (wrap.position() == 0 && !this.f71406d.hasRemaining()) {
                        z = true;
                        break;
                    }
                    this.f71405c.reset();
    ...
    /* renamed from: b */
    public final int m26936b() {
        int i;
        this.f71406d.compact();
        try {
            int limit = this.f71406d.limit();
            int position = this.f71406d.position();
            if (position <= limit) {
                i = limit - position;
            } else {
                i = 0;
            }
            int read = this.f71403a.read(this.f71406d.array(), this.f71406d.arrayOffset() + position, i);
            if (read < 0) {
                return read;
            }
            ByteBuffer byteBuffer = this.f71406d;
            Intrinsics.checkNotNull(byteBuffer, "null cannot be cast to non-null type java.nio.Buffer");
            byteBuffer.position(position + read);
            this.f71406d.flip();
            return this.f71406d.remaining();
        } finally {
            this.f71406d.flip();
        }
    }
    ...
    public final int read(@NotNull char[] array, int i, int i2) {
        ...
        if (i >= 0 && i < array.length && i2 >= 0 && i + i2 <= array.length) {
            ...
            if (i2 == 1) {
                ...
                array[i] = (char) m26935c;
                return i3 + 1;
            }
            return m26937a(array, i, i2) + i3;
        }
