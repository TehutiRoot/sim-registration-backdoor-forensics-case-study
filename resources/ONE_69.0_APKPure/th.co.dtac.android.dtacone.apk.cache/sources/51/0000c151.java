...
public class LayerUtility {
    ...
    /* renamed from: c */
    public static final Set f59944c = new HashSet(Arrays.asList(PDAnnotationMarkup.RT_GROUP, "LastModified", "Metadata"));

    /* renamed from: a */
    public final PDDocument f59945a;

    /* renamed from: b */
    public final PDFCloneUtility f59946b;

    public LayerUtility(PDDocument pDDocument) {
        this.f59945a = pDDocument;
        this.f59946b = new PDFCloneUtility(pDDocument);
    }

    /* renamed from: a */
    public final void m32740a(PDDocument pDDocument) {
        PDOptionalContentProperties oCProperties = pDDocument.getDocumentCatalog().getOCProperties();
        if (oCProperties == null) {
            return;
        }
        PDDocumentCatalog documentCatalog = this.f59945a.getDocumentCatalog();
        PDOptionalContentProperties oCProperties2 = documentCatalog.getOCProperties();
        if (oCProperties2 == null) {
            documentCatalog.setOCProperties(new PDOptionalContentProperties((COSDictionary) this.f59946b.cloneForNewDocument(oCProperties)));
        } else {
            this.f59946b.cloneMerge(oCProperties, oCProperties2);
        }
    }
    ...
    public PDOptionalContentGroup appendFormAsLayer(PDPage pDPage, PDFormXObject pDFormXObject, AffineTransform affineTransform, String str) throws IOException {
        ...
        if (!oCProperties.hasGroup(str)) {
            ...
            PDPageContentStream pDPageContentStream = new PDPageContentStream(this.f59945a, pDPage, PDPageContentStream.AppendMode.APPEND, false);
            pDPageContentStream.beginMarkedContent(COSName.f59822OC, pDOptionalContentGroup);
    ...
    /* renamed from: b */
    public final void m32739b(COSDictionary cOSDictionary, COSDictionary cOSDictionary2, Set set) {
        for (Map.Entry<COSName, COSBase> entry : cOSDictionary.entrySet()) {
            COSName key = entry.getKey();
            if (set.contains(key.getName())) {
                cOSDictionary2.setItem(key, this.f59946b.cloneForNewDocument(entry.getValue()));
            }
        }
    }
    ...
    public PDFormXObject importPageAsForm(PDDocument pDDocument, PDPage pDPage) throws IOException {
        m32740a(pDDocument);
        PDFormXObject pDFormXObject = new PDFormXObject(new PDStream(this.f59945a, pDPage.getContents(), COSName.FLATE_DECODE));
        PDResources resources = pDPage.getResources();
        PDResources pDResources = new PDResources();
        this.f59946b.cloneMerge(resources, pDResources);
        pDFormXObject.setResources(pDResources);
        m32739b(pDPage.getCOSObject(), pDFormXObject.getCOSObject(), f59944c);
