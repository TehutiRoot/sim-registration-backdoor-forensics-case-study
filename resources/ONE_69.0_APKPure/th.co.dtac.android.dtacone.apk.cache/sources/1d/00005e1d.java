...
public final class PersistentHashMapBuilderEntries<K, V> extends AbstractMapBuilderEntries<Map.Entry<K, V>, K, V> {
    ...
    public boolean containsEntry(@NotNull Map.Entry<? extends K, ? extends V> element) {
        ...
        Object obj = this.f28654a.get(element.getKey());
        if (obj != null) {
            return Intrinsics.areEqual(obj, element.getValue());
        }
        if (element.getValue() == null && this.f28654a.containsKey(element.getKey())) {
            return true;
        }
        return false;
    }
    ...
    public Iterator<Map.Entry<K, V>> iterator() {
        return new PersistentHashMapBuilderEntriesIterator(this.f28654a);
    }

    @Override // androidx.compose.runtime.external.kotlinx.collections.immutable.implementations.immutableMap.AbstractMapBuilderEntries
    public boolean removeEntry(@NotNull Map.Entry<? extends K, ? extends V> element) {
        Intrinsics.checkNotNullParameter(element, "element");
        return this.f28654a.remove(element.getKey(), element.getValue());
    }
