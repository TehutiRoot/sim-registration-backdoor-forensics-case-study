...
public final class CipherSink implements Sink {
    ...
    /* renamed from: b */
    public final Throwable m26036b() {
        int outputSize = this.f72884b.getOutputSize(0);
        Throwable th2 = null;
        if (outputSize == 0) {
            return null;
        }
        if (outputSize > 8192) {
            try {
                BufferedSink bufferedSink = this.f72883a;
                byte[] doFinal = this.f72884b.doFinal();
        ...
        Buffer buffer = this.f72883a.getBuffer();
        Segment writableSegment$okio = buffer.writableSegment$okio(outputSize);
        try {
            int doFinal2 = this.f72884b.doFinal(writableSegment$okio.data, writableSegment$okio.limit);
    ...
    /* renamed from: d */
    public final int m26035d(Buffer buffer, long j) {
        Segment segment = buffer.head;
        Intrinsics.checkNotNull(segment);
        int min = (int) Math.min(j, segment.limit - segment.pos);
        Buffer buffer2 = this.f72883a.getBuffer();
        int outputSize = this.f72884b.getOutputSize(min);
        while (outputSize > 8192) {
            int i = this.f72885c;
            if (min <= i) {
                BufferedSink bufferedSink = this.f72883a;
                byte[] update = this.f72884b.update(buffer.readByteArray(j));
                Intrinsics.checkNotNullExpressionValue(update, "cipher.update(source.readByteArray(remaining))");
                bufferedSink.write(update);
                return (int) j;
            }
            min -= i;
            outputSize = this.f72884b.getOutputSize(min);
        }
        Segment writableSegment$okio = buffer2.writableSegment$okio(outputSize);
        int update2 = this.f72884b.update(segment.data, segment.pos, min, writableSegment$okio.data, writableSegment$okio.limit);
        ...
        this.f72883a.emitCompleteSegments();
