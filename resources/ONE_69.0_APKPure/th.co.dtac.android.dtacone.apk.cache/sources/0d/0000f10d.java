...
public class RequestAuthCache implements HttpRequestInterceptor {
    ...
    /* renamed from: a */
    public final Log f73807a = LogFactory.getLog(getClass());

    /* renamed from: a */
    public final void m25018a(HttpHost httpHost, AuthScheme authScheme, AuthState authState, CredentialsProvider credentialsProvider) {
        String schemeName = authScheme.getSchemeName();
        if (this.f73807a.isDebugEnabled()) {
            Log log = this.f73807a;
            log.debug("Re-using cached '" + schemeName + "' auth scheme for " + httpHost);
        }
        Credentials credentials = credentialsProvider.getCredentials(new AuthScope(httpHost, AuthScope.ANY_REALM, schemeName));
        if (credentials != null) {
            authState.update(authScheme, credentials);
        } else {
            this.f73807a.debug("No credentials for preemptive authentication");
        }
    }
    ...
    public void process(HttpRequest httpRequest, HttpContext httpContext) throws HttpException, IOException {
        ...
        if (authCache == null) {
            this.f73807a.debug("Auth cache not set in the context");
            return;
        }
        CredentialsProvider credentialsProvider = adapt.getCredentialsProvider();
        if (credentialsProvider == null) {
            this.f73807a.debug("Credentials provider not set in the context");
            return;
        }
        RouteInfo httpRoute = adapt.getHttpRoute();
        if (httpRoute == null) {
            this.f73807a.debug("Route info not set in the context");
            return;
        }
        HttpHost targetHost = adapt.getTargetHost();
        if (targetHost == null) {
            this.f73807a.debug("Target host not set in the context");
            return;
        }
        ...
        if (targetAuthState != null && targetAuthState.getState() == AuthProtocolState.UNCHALLENGED && (authScheme2 = authCache.get(targetHost)) != null) {
            m25018a(targetHost, authScheme2, targetAuthState, credentialsProvider);
        }
        ...
        if (proxyHost != null && proxyAuthState != null && proxyAuthState.getState() == AuthProtocolState.UNCHALLENGED && (authScheme = authCache.get(proxyHost)) != null) {
            m25018a(proxyHost, authScheme, proxyAuthState, credentialsProvider);
        }
    }
}