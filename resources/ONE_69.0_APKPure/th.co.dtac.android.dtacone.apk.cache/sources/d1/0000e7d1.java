...
public final class FloatArrayBuilder extends PrimitiveArrayBuilder<float[]> {
    ...
    public FloatArrayBuilder(@NotNull float[] bufferWithData) {
        ...
        this.f71232a = bufferWithData;
        this.f71233b = bufferWithData.length;
        ensureCapacity$kotlinx_serialization_core(10);
    }

    public final void append$kotlinx_serialization_core(float f) {
        PrimitiveArrayBuilder.ensureCapacity$kotlinx_serialization_core$default(this, 0, 1, null);
        float[] fArr = this.f71232a;
        int position$kotlinx_serialization_core = getPosition$kotlinx_serialization_core();
        this.f71233b = position$kotlinx_serialization_core + 1;
    ...
    public void ensureCapacity$kotlinx_serialization_core(int i) {
        float[] fArr = this.f71232a;
        if (fArr.length < i) {
            float[] copyOf = Arrays.copyOf(fArr, AbstractC11695c.coerceAtLeast(i, fArr.length * 2));
    ...
    public float[] build$kotlinx_serialization_core() {
        float[] copyOf = Arrays.copyOf(this.f71232a, getPosition$kotlinx_serialization_core());
        Intrinsics.checkNotNullExpressionValue(copyOf, "copyOf(...)");
        return copyOf;
    }
}