...
public final class BringIntoViewRequestPriorityQueue {
    ...
    /* renamed from: a */
    public final MutableVector f13017a = new MutableVector(new ContentInViewModifier.Request[16], 0);

    public final void cancelAndRemoveAll(@Nullable Throwable th2) {
        MutableVector mutableVector = this.f13017a;
    ...
    public final boolean enqueue(@NotNull ContentInViewModifier.Request request) {
        ...
        IntRange intRange = new IntRange(0, this.f13017a.getSize() - 1);
        int first = intRange.getFirst();
        int last = intRange.getLast();
        if (first <= last) {
            while (true) {
                Rect invoke2 = ((ContentInViewModifier.Request) this.f13017a.getContent()[last]).getCurrentBounds().invoke();
                if (invoke2 != null) {
                    Rect intersect = invoke.intersect(invoke2);
                    if (Intrinsics.areEqual(intersect, invoke)) {
                        this.f13017a.add(last + 1, request);
                        return true;
                    } else if (!Intrinsics.areEqual(intersect, invoke2)) {
                        CancellationException cancellationException = new CancellationException("bringIntoView call interrupted by a newer, non-overlapping call");
                        int size = this.f13017a.getSize() - 1;
                        if (size <= last) {
                            while (true) {
                                ((ContentInViewModifier.Request) this.f13017a.getContent()[last]).getContinuation().cancel(cancellationException);
                                if (size == last) {
                                    break;
                                }
                                size++;
    ...
    public final void resumeAndRemoveAll() {
        ...
        if (first <= last) {
            while (true) {
                ((ContentInViewModifier.Request) this.f13017a.getContent()[first]).getContinuation().resumeWith(Result.m74271constructorimpl(Unit.INSTANCE));
                if (first == last) {
                    break;
                }
                first++;
    ...
    public final void resumeAndRemoveWhile(@NotNull Function1<? super Rect, Boolean> block) {
        Intrinsics.checkNotNullParameter(block, "block");
        while (this.f13017a.isNotEmpty() && block.invoke(((ContentInViewModifier.Request) this.f13017a.last()).getCurrentBounds().invoke()).booleanValue()) {
            ((ContentInViewModifier.Request) this.f13017a.removeAt(this.f13017a.getSize() - 1)).getContinuation().resumeWith(Result.m74271constructorimpl(Unit.INSTANCE));
        }
    }
}