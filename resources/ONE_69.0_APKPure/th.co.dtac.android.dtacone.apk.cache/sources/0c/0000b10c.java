...
public class zzf {
    ...
    public MappedByteBuffer load() throws FirebaseMLException {
        String str;
        Preconditions.checkNotNull(this.f55776b, "Context can not be null");
        Preconditions.checkNotNull(this.f55777c, "Model source can not be null");
        MappedByteBuffer mappedByteBuffer = this.f55775a;
        if (mappedByteBuffer != null) {
            return mappedByteBuffer;
        }
        if (this.f55777c.getFilePath() != null) {
            try {
                RandomAccessFile randomAccessFile = new RandomAccessFile(this.f55777c.getFilePath(), PDPageLabelRange.STYLE_ROMAN_LOWER);
                FileChannel channel = randomAccessFile.getChannel();
                try {
                    this.f55775a = channel.map(FileChannel.MapMode.READ_ONLY, 0L, channel.size());
                    channel.close();
                ...
                String valueOf = String.valueOf(this.f55777c.getFilePath());
                if (valueOf.length() != 0) {
        ...
        } else if (this.f55777c.getAssetFilePath() != null) {
            String assetFilePath = this.f55777c.getAssetFilePath();
            try {
                AssetFileDescriptor openFd = this.f55776b.getAssets().openFd(assetFilePath);
                FileInputStream fileInputStream = new FileInputStream(openFd.getFileDescriptor());
                FileChannel channel2 = fileInputStream.getChannel();
                try {
                    this.f55775a = channel2.map(FileChannel.MapMode.READ_ONLY, openFd.getStartOffset(), openFd.getDeclaredLength());
                    channel2.close();
