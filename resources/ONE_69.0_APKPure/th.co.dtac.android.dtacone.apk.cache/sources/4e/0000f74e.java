...
public class Tables8kGCMMultiplier implements GCMMultiplier {
    ...
    public void init(byte[] bArr) {
        if (this.f75244T == null) {
            this.f75244T = (long[][][]) Array.newInstance(Long.TYPE, 32, 16, 2);
        } else if (Arrays.areEqual(this.f75243H, bArr)) {
            return;
        }
        this.f75243H = Arrays.clone(bArr);
        for (int i = 0; i < 32; i++) {
            long[][][] jArr = this.f75244T;
            long[][] jArr2 = jArr[i];
            if (i == 0) {
                GCMUtil.asLongs(this.f75243H, jArr2[1]);
                long[] jArr3 = jArr2[1];
