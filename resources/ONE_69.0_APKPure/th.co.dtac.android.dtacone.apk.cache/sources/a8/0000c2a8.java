    ...
    public void generateNormalAppearance() {
        ...
        try {
            try {
                ...
                pDAppearanceContentStream.setStrokingColor(m32338n());
                pDAppearanceContentStream.setNonStrokingColor(m32338n());
                m32331u(pDAppearanceContentStream, pDAnnotationMarkup.getConstantOpacity());
                PDRectangle m32333s = m32333s();
                PDRectangle pDRectangle = new PDRectangle(m32333s.getWidth(), m32333s.getHeight());
                if (!pDAnnotationMarkup.getCOSObject().containsKey(COSName.f59833RD)) {
                    float min = Math.min(m32333s.getHeight() / 10.0f, 5.0f);
                    pDAnnotationMarkup.setRectDifferences(min);
                    float f = -min;
                    float f2 = min * 2.0f;
                    PDRectangle pDRectangle2 = new PDRectangle(f, f, m32333s.getWidth() + f2, m32333s.getHeight() + f2);
                    ...
                    pDAnnotationMarkup.setRectangle(new PDRectangle(m32333s.getLowerLeftX() - min, m32333s.getLowerLeftY() - min, m32333s.getWidth() + f2, m32333s.getHeight() + f2));
                    pDRectangle = pDRectangle2;
                }
                pDAnnotationMarkup.getNormalAppearanceStream().setBBox(pDRectangle);
                float width = m32333s.getWidth() / 2.0f;
                float height = m32333s.getHeight() / 2.0f;
                pDAppearanceContentStream.moveTo(0.0f, 0.0f);
                pDAppearanceContentStream.curveTo(width, 0.0f, width, height, width, m32333s.getHeight());
                pDAppearanceContentStream.curveTo(width, height, width, 0.0f, m32333s.getWidth(), 0.0f);
