...
public class ChromeDevtoolsServer implements SimpleEndpoint {
    ...
    private void handleRemoteRequest(JsonRpcPeer jsonRpcPeer, JSONObject jSONObject) throws MessageHandlingException {
        ...
        if (jsonRpcRequest.f43096id != null) {
            JsonRpcResponse jsonRpcResponse = new JsonRpcResponse();
            jsonRpcResponse.f43097id = jsonRpcRequest.f43096id.longValue();
    ...
    private void handleRemoteResponse(JsonRpcPeer jsonRpcPeer, JSONObject jSONObject) throws MismatchedResponseException {
        ...
        PendingRequest andRemovePendingRequest = jsonRpcPeer.getAndRemovePendingRequest(jsonRpcResponse.f43097id);
        ...
        throw new MismatchedResponseException(jsonRpcResponse.f43097id);
    }
    ...
    private static void logDispatchException(JsonRpcException jsonRpcException) {
        ...
        if (C59971.f43095x978b462[errorMessage.code.ordinal()] != 1) {
            LogRedirector.m49805w(TAG, "Error processing remote message", jsonRpcException);
            return;
        }
        LogRedirector.m49814d(TAG, "Method not implemented: " + errorMessage.message);
    }

    @Override // com.facebook.stetho.websocket.SimpleEndpoint
    public void onClose(SimpleSession simpleSession, int i, String str) {
        LogRedirector.m49814d(TAG, "onClose: reason=" + i + HelpFormatter.DEFAULT_LONG_OPT_SEPARATOR + str);
    ...
    public void onError(SimpleSession simpleSession, Throwable th2) {
        LogRedirector.m49812e(TAG, "onError: ex=" + th2.toString());
    }

    @Override // com.facebook.stetho.websocket.SimpleEndpoint
    public void onMessage(SimpleSession simpleSession, byte[] bArr, int i) {
        LogRedirector.m49814d(TAG, "Ignoring binary message of length " + i);
    }

    @Override // com.facebook.stetho.websocket.SimpleEndpoint
    public void onOpen(SimpleSession simpleSession) {
        LogRedirector.m49814d(TAG, "onOpen");
    ...
    public void onMessage(SimpleSession simpleSession, String str) {
        if (LogRedirector.isLoggable(TAG, 2)) {
            LogRedirector.m49808v(TAG, "onMessage: message=" + str);
        }
        try {
            ...
            LogRedirector.m49810i(TAG, "Message could not be processed by implementation: " + e);
            closeSafely(simpleSession, 1011, e.getClass().getSimpleName());
        } catch (IOException e2) {
            if (LogRedirector.isLoggable(TAG, 2)) {
                LogRedirector.m49808v(TAG, "Unexpected I/O exception processing message: " + e2);
            }
            closeSafely(simpleSession, 1011, e2.getClass().getSimpleName());
        } catch (JSONException e3) {
            LogRedirector.m49807v(TAG, "Unexpected JSON exception processing message", e3);
