...
public abstract class AH1 extends AbstractC23354zH1 {
    @NotNull
    ...
    public static final <T> HashSet<T> hashSetOf(@NotNull T... elements) {
        ...
        return (HashSet) ArraysKt___ArraysKt.toCollection(elements, new HashSet(AbstractC18946Zn0.mapCapacity(elements.length)));
    }

    @NotNull
    public static <T> LinkedHashSet<T> linkedSetOf(@NotNull T... elements) {
        Intrinsics.checkNotNullParameter(elements, "elements");
        return (LinkedHashSet) ArraysKt___ArraysKt.toCollection(elements, new LinkedHashSet(AbstractC18946Zn0.mapCapacity(elements.length)));
    }

    @NotNull
    public static <T> Set<T> mutableSetOf(@NotNull T... elements) {
        Intrinsics.checkNotNullParameter(elements, "elements");
        return (Set) ArraysKt___ArraysKt.toCollection(elements, new LinkedHashSet(AbstractC18946Zn0.mapCapacity(elements.length)));
    }
    ...
    public static final <T> Set<T> optimizeReadOnlySet(@NotNull Set<? extends T> set) {
        ...
        if (size != 0) {
            if (size == 1) {
                return AbstractC23354zH1.setOf(set.iterator().next());
            }
            return set;
        }
        return emptySet();
    ...
    public static final <T> Set<T> setOfNotNull(@Nullable T t) {
        return t != null ? AbstractC23354zH1.setOf(t) : emptySet();
    }
