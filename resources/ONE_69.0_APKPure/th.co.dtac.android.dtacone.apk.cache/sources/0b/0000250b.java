...
public final class SnapFlingBehaviorKt$animateDecay$2 extends Lambda implements Function1<AnimationScope<Float, AnimationVector1D>, Unit> {
    ...
    public final void invoke2(@NotNull AnimationScope<Float, AnimationVector1D> animateDecay) {
        ...
        if (Math.abs(animateDecay.getValue().floatValue()) >= Math.abs(this.$targetOffset)) {
            m61390f = SnapFlingBehaviorKt.m61390f(animateDecay.getValue().floatValue(), this.$targetOffset);
            SnapFlingBehaviorKt.m61394b(animateDecay, this.$this_animateDecay, this.$onAnimationStep, m61390f - this.$previousValue.element);
            animateDecay.cancelAnimation();
            this.$previousValue.element = m61390f;
            return;
        }
        SnapFlingBehaviorKt.m61394b(animateDecay, this.$this_animateDecay, this.$onAnimationStep, animateDecay.getValue().floatValue() - this.$previousValue.element);
