...
public abstract class DeserializationContext extends DatabindContext implements Serializable {
    ...
    public final JsonDeserializer<Object> findRootValueDeserializer(JavaType javaType) throws JsonMappingException {
        ...
        if (findTypeDeserializer != null) {
            return new TypeWrappedDeserializer(findTypeDeserializer.forProperty(null), handleSecondaryContextualization);
        }
    ...
    public boolean handleUnknownProperty(JsonParser jsonParser, JsonDeserializer<?> jsonDeserializer, Object obj, String str) throws IOException {
        ...
        if (jsonDeserializer == null) {
            ...
            knownPropertyNames = jsonDeserializer.getKnownPropertyNames();
        }
        throw UnrecognizedPropertyException.from(this._parser, obj, str, knownPropertyNames);
    }
    ...
    public JsonMappingException instantiationException(Class<?> cls, Throwable th2) {
        InvalidDefinitionException from = InvalidDefinitionException.from(this._parser, String.format("Cannot construct instance of %s, problem: %s", ClassUtil.nameOf(cls), th2.getMessage()), constructType(cls));
        from.initCause(th2);
        return from;
    }
    ...
    public <T> T readPropertyValue(JsonParser jsonParser, BeanProperty beanProperty, JavaType javaType) throws IOException {
        ...
        return findContextualValueDeserializer == null ? (T) reportBadDefinition(javaType, String.format("Could not find JsonDeserializer for type %s (via property %s)", javaType, ClassUtil.nameOf(beanProperty))) : (T) findContextualValueDeserializer.deserialize(jsonParser, this);
    }
