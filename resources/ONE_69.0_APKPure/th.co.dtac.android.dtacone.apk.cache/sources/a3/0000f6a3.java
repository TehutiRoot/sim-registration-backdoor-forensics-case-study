...
public class NaccacheSternEngine implements AsymmetricBlockCipher {
                    ...
                    for (int i3 = 0; i3 < smallPrimes.size(); i3++) {
                        ...
                        if (vector2.size() != ((BigInteger) smallPrimes.elementAt(i3)).intValue()) {
                            if (this.debug) {
                                ...
                                printStream2.println("Prime is " + smallPrimes.elementAt(i3) + ", lookup table has size " + vector2.size());
                            }
                        ...
                        if (indexOf == -1) {
                            if (this.debug) {
                                ...
                                printStream3.println("Actual prime is " + smallPrimes.elementAt(i3));
                                printStream3.println("Decrypted value is " + modPow);
                                printStream3.println("LookupList for " + smallPrimes.elementAt(i3) + " with size " + this.lookup[i3].size() + " is: ");
    ...
    public byte[] processData(byte[] bArr) throws InvalidCipherTextException {
        ...
        while (i < bArr.length) {
            ...
            if (this.debug) {
                PrintStream printStream2 = System.out;
                printStream2.println("new datapos is " + i);
            }
        ...
        if (this.debug) {
            PrintStream printStream3 = System.out;
            printStream3.println("returning " + i2 + " bytes");
        }
        return bArr3;
    }
