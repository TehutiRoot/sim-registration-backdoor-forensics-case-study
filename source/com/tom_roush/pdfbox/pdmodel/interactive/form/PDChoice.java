...
public abstract class PDChoice extends PDVariableText {
    public PDChoice(PDAcroForm pDAcroForm) {
        super(pDAcroForm);
        getCOSObject().setItem(COSName.f59786FT, (COSBase) COSName.f59764CH);
    }

    /* renamed from: e */
    public final List m32240e(COSName cOSName) {
    ...
    /* renamed from: f */
    public final void m32239f(List list) {
    ...
    public List<String> getDefaultValue() {
        return m32240e(COSName.f59778DV);
    }

    public List<String> getOptions() {
        return FieldUtils.m32259a(getCOSObject().getDictionaryObject(COSName.OPT), 0);
    }

    public List<String> getOptionsDisplayValues() {
        return FieldUtils.m32259a(getCOSObject().getDictionaryObject(COSName.OPT), 1);
    }
    ...
    public List<String> getValue() {
        return m32240e(COSName.f59843V);
    }
    ...
    public boolean isCombo() {
        return getCOSObject().getFlag(COSName.f59784FF, 131072);
    }

    public boolean isCommitOnSelChange() {
        return getCOSObject().getFlag(COSName.f59784FF, 67108864);
    }
    ...
    public boolean isMultiSelect() {
        return getCOSObject().getFlag(COSName.f59784FF, 2097152);
    }

    public boolean isSort() {
        return getCOSObject().getFlag(COSName.f59784FF, 524288);
    }

    public void setCombo(boolean z) {
        getCOSObject().setFlag(COSName.f59784FF, 131072, z);
    }

    public void setCommitOnSelChange(boolean z) {
        getCOSObject().setFlag(COSName.f59784FF, 67108864, z);
    }
    ...
    public void setMultiSelect(boolean z) {
        getCOSObject().setFlag(COSName.f59784FF, 2097152, z);
    }
    ...
    public void setSelectedOptionsIndex(List<Integer> list) {
        if (list != null && !list.isEmpty()) {
            if (isMultiSelect()) {
                getCOSObject().setItem(COSName.f59789I, (COSBase) COSArrayList.converterToCOSArray(list));
                return;
            }
            throw new IllegalArgumentException("Setting the indices is not allowed for choice fields not allowing multiple selections.");
        }
        getCOSObject().removeItem(COSName.f59789I);
    }

    public void setSort(boolean z) {
        getCOSObject().setFlag(COSName.f59784FF, 524288, z);
    }
        ...
        if (list != null && !list.isEmpty()) {
            if (isMultiSelect()) {
                if (getOptions().containsAll(list)) {
                    getCOSObject().setItem(COSName.f59843V, (COSBase) COSArrayList.convertStringListToCOSStringCOSArray(list));
                    m32239f(list);
                } else {
            ...
            getCOSObject().removeItem(COSName.f59843V);
            getCOSObject().removeItem(COSName.f59789I);
        }
        applyChange();
    }
    ...
    public void setOptions(List<String> list, List<String> list2) {
        if (list != null && list2 != null && !list.isEmpty() && !list2.isEmpty()) {
            if (list.size() == list2.size()) {
                List m32257c = FieldUtils.m32257c(list, list2);
                if (isSort()) {
                    FieldUtils.m32258b(m32257c);
                }
                COSArray cOSArray = new COSArray();
                for (int i = 0; i < list.size(); i++) {
                    COSArray cOSArray2 = new COSArray();
                    cOSArray2.add((COSBase) new COSString(((FieldUtils.C9958a) m32257c.get(i)).m32254c()));
                    cOSArray2.add((COSBase) new COSString(((FieldUtils.C9958a) m32257c.get(i)).m32253d()));
