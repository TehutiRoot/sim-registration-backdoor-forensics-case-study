...
public class PDPolygonAppearanceHandler extends PDAbstractAppearanceHandler {
    ...
    public void generateNormalAppearance() {
        ...
        float m32333w = m32333w();
        PDRectangle rectangle = pDAnnotationMarkup.getRectangle();
        float[][] m32332x = m32332x(pDAnnotationMarkup);
        if (m32332x == null) {
            return;
        }
        ...
        rectangle.setLowerLeftX(Math.min(f - m32333w, rectangle.getLowerLeftX()));
        rectangle.setLowerLeftY(Math.min(f2 - m32333w, rectangle.getLowerLeftY()));
        rectangle.setUpperRightX(Math.max(f3 + m32333w, rectangle.getUpperRightX()));
        rectangle.setUpperRightY(Math.max(f4 + m32333w, rectangle.getUpperRightY()));
        ...
        try {
            try {
                ...
                pDAppearanceContentStream.setBorderLine(m32333w, pDAnnotationMarkup.getBorderStyle(), pDAnnotationMarkup.getBorder());
                ...
                if (borderEffect != null && borderEffect.getStyle().equals("C")) {
                    C12399nq c12399nq = new C12399nq(pDAppearanceContentStream, borderEffect.getIntensity(), m32333w, m32341s());
                    c12399nq.m26023l(m32332x);
                    pDAnnotationMarkup.setRectangle(c12399nq.m26044B());
                    ...
                    for (int i3 = 0; i3 < m32332x.length; i3++) {
                        float[] fArr2 = m32332x[i3];
                ...
                pDAppearanceContentStream.drawShape(m32333w, strokingColorOnDemand, nonStrokingColorOnDemand);
            } catch (IOException e) {
                e.getMessage();
            }
    ...
    /* renamed from: x */
    public final float[][] m32332x(PDAnnotationMarkup pDAnnotationMarkup) {
