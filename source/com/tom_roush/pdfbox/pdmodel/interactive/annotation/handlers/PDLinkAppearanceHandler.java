...
public class PDLinkAppearanceHandler extends PDAbstractAppearanceHandler {
    ...
    public void generateNormalAppearance() {
        ...
        PDAnnotationLink pDAnnotationLink = (PDAnnotationLink) m32349k();
        PDRectangle rectangle = pDAnnotationLink.getRectangle();
        if (rectangle == null) {
            return;
        }
        float m32334w = m32334w();
        try {
            m32344p = m32344p();
            try {
                ...
                strokingColorOnDemand = m32344p.setStrokingColorOnDemand(color);
                m32344p.setBorderLine(m32334w, pDAnnotationLink.getBorderStyle(), pDAnnotationLink.getBorder());
            } catch (IOException e) {
                e = e;
            } catch (Throwable th2) {
                th = th2;
        ...
        try {
            PDRectangle m32342r = m32342r(m32341s(), m32334w / 2.0f);
            ...
            if (quadPoints == null) {
                quadPoints = new float[]{m32342r.getLowerLeftX(), m32342r.getLowerLeftY(), m32342r.getUpperRightX(), m32342r.getLowerLeftY(), m32342r.getUpperRightX(), m32342r.getUpperRightY(), m32342r.getLowerLeftX(), m32342r.getUpperRightY()};
            }
            ...
            while (true) {
                ...
                if (i5 < quadPoints.length) {
                    m32344p.moveTo(quadPoints[i4], quadPoints[i4 + 1]);
                    m32344p.lineTo(quadPoints[i4 + 2], quadPoints[i4 + 3]);
                    if (!z) {
                        m32344p.lineTo(quadPoints[i4 + 4], quadPoints[i4 + 5]);
                        m32344p.lineTo(quadPoints[i4 + 6], quadPoints[i5]);
                        m32344p.closePath();
                    }
                    i4 += 8;
                } else {
                    m32344p.drawShape(m32334w, strokingColorOnDemand, false);
