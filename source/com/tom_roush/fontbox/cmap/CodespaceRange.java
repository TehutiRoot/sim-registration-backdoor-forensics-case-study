...
public class CodespaceRange {
    ...
    public CodespaceRange(byte[] bArr, byte[] bArr2) {
        ...
        this.f59358c = new int[bArr.length];
        this.f59359d = new int[bArr2.length];
        for (int i = 0; i < bArr.length; i++) {
            this.f59358c[i] = bArr[i] & 255;
            this.f59359d[i] = bArr2[i] & 255;
        }
        this.f59360e = bArr2.length;
    }
    ...
    public boolean isFullMatch(byte[] bArr, int i) {
        ...
        for (int i2 = 0; i2 < this.f59360e; i2++) {
            int i3 = bArr[i2] & 255;
            if (i3 < this.f59358c[i2] || i3 > this.f59359d[i2]) {
                return false;
            }
        }
        return true;
