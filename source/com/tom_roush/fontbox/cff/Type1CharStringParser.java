...
public class Type1CharStringParser {
    ...
    /* renamed from: d */
    public static Integer m33009d(List list) {
    ...
    /* renamed from: a */
    public final List m33012a(byte[] bArr, List list, List list2) {
        ...
        while (dataInput.hasRemaining()) {
            ...
            if (readUnsignedByte == 10) {
                ...
                if (!(remove instanceof Integer)) {
                    Log.w("PdfBox-Android", "Parameter " + remove + " for CALLSUBR is ignored, integer expected in glyph '" + this.f59325b + "' of font " + this.f59324a);
                } else {
                    Integer num = (Integer) remove;
                    if (num.intValue() >= 0 && num.intValue() < list.size()) {
                        m33012a((byte[]) list.get(num.intValue()), list, list2);
                        Object obj = list2.get(list2.size() - 1);
                        if ((obj instanceof CharStringCommand) && ((CharStringCommand) obj).getKey().getValue()[0] == 11) {
                            list2.remove(list2.size() - 1);
                        }
                    } else {
                        Log.w("PdfBox-Android", "CALLSUBR is ignored, operand: " + num + ", subrs.size(): " + list.size() + " in glyph '" + this.f59325b + "' of font " + this.f59324a);
                ...
                if (arrayDeque.size() > 0) {
                    Log.w("PdfBox-Android", "Value left on the PostScript stack in glyph " + this.f59325b + " of font " + this.f59324a);
                }
            } else if (readUnsignedByte >= 0 && readUnsignedByte <= 31) {
                list2.add(m33011b(dataInput, readUnsignedByte));
            } else if (readUnsignedByte >= 32 && readUnsignedByte <= 255) {
                list2.add(m33010c(dataInput, readUnsignedByte));
            } else {
    ...
    /* renamed from: b */
    public final CharStringCommand m33011b(DataInput dataInput, int i) {
        if (i == 12) {
    ...
    /* renamed from: c */
    public final Integer m33010c(DataInput dataInput, int i) {
        if (i >= 32 && i <= 246) {
    ...
    public List<Object> parse(byte[] bArr, List<byte[]> list) throws IOException {
        return m33012a(bArr, list, new ArrayList());
    }
}
