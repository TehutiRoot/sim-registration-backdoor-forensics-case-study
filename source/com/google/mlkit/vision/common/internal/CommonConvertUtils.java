...
public class CommonConvertUtils {
    /* renamed from: a */
    public static void m37191a(int i, int i2, long j, int i3, int i4, int i5, int i6) {
    ...
    public static InputImage convertMlImagetoInputImage(@NonNull MlImage mlImage) {
        ...
        if (storageType != 1) {
            ...
            if (storageType != 2) {
                if (storageType == 3) {
                    Image extract = MediaImageExtractor.extract(mlImage);
                    if (extract.getFormat() == 256) {
                        limit = extract.getPlanes()[0].getBuffer().limit();
                    } else {
                        limit = (extract.getPlanes()[0].getBuffer().limit() * 3) / 2;
                    }
                    m37191a(extract.getFormat(), 5, SystemClock.elapsedRealtime(), mlImage.getHeight(), mlImage.getWidth(), limit, mlImage.getRotation());
                    fromBitmap = InputImage.fromMediaImage(extract, mlImage.getRotation());
                }
            } else {
                ...
                if (num != null) {
                    m37191a(num.intValue(), 3, SystemClock.elapsedRealtime(), mlImage.getHeight(), mlImage.getWidth(), extract2.limit(), mlImage.getRotation());
                    fromBitmap = InputImage.fromByteBuffer(extract2, mlImage.getWidth(), mlImage.getHeight(), mlImage.getRotation(), num.intValue());
                }
            }
        } else {
            Bitmap extract3 = BitmapExtractor.extract(mlImage);
            m37191a(-1, 1, SystemClock.elapsedRealtime(), mlImage.getHeight(), mlImage.getWidth(), extract3.getAllocationByteCount(), mlImage.getRotation());
