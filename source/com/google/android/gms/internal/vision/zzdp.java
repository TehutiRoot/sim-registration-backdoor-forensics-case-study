package com.google.android.gms.internal.vision;

import java.io.Serializable;
import java.util.AbstractMap;
import java.util.Arrays;
import java.util.Collection;
import java.util.Iterator;
import java.util.LinkedHashMap;
import java.util.Map;
import java.util.Set;
import kotlinx.coroutines.internal.LockFreeTaskQueueCore;
import org.checkerframework.checker.nullness.compatqual.NullableDecl;

/* JADX INFO: Access modifiers changed from: package-private */
/* loaded from: classes3.dex */
public final class zzdp<K, V> extends AbstractMap<K, V> implements Serializable {
    private static final Object zzd = new Object();
    @NullableDecl
    transient int[] zza;
    @NullableDecl
    transient Object[] zzb;
    @NullableDecl
    transient Object[] zzc;
    @NullableDecl
    private transient Object zze;
    private transient int zzf;
    private transient int zzg;
    @NullableDecl
    private transient Set<K> zzh;
    @NullableDecl
    private transient Set<Map.Entry<K, V>> zzi;
    @NullableDecl
    private transient Collection<V> zzj;

    public zzdp() {
        zzde.zza(true, (Object) "Expected size must be >= 0");
        this.zzf = zzfc.zza(3, 1, LockFreeTaskQueueCore.MAX_CAPACITY_MASK);
    }

    public static int zzb(int i, int i2) {
        return i - 1;
    }

    /* JADX INFO: Access modifiers changed from: private */
    public final int zzi() {
        return (1 << (this.zzf & 31)) - 1;
    }

    @Override // java.util.AbstractMap, java.util.Map
    public final void clear() {
        if (zza()) {
            return;
        }
        zzc();
        Map<K, V> zzb = zzb();
        if (zzb != null) {
            this.zzf = zzfc.zza(size(), 3, LockFreeTaskQueueCore.MAX_CAPACITY_MASK);
            zzb.clear();
            this.zze = null;
            this.zzg = 0;
            return;
        }
        Arrays.fill(this.zzb, 0, this.zzg, (Object) null);
        Arrays.fill(this.zzc, 0, this.zzg, (Object) null);
        Object obj = this.zze;
        if (obj instanceof byte[]) {
            Arrays.fill((byte[]) obj, (byte) 0);
        } else if (obj instanceof short[]) {
            Arrays.fill((short[]) obj, (short) 0);
        } else {
            Arrays.fill((int[]) obj, 0);
        }
        Arrays.fill(this.zza, 0, this.zzg, 0);
        this.zzg = 0;
    }

    @Override // java.util.AbstractMap, java.util.Map
    public final boolean containsKey(@NullableDecl Object obj) {
        Map<K, V> zzb = zzb();
        if (zzb != null) {
            return zzb.containsKey(obj);
        }
        if (zza(obj) != -1) {
            return true;
        }
        return false;
    }

    @Override // java.util.AbstractMap, java.util.Map
    public final boolean containsValue(@NullableDecl Object obj) {
        Map<K, V> zzb = zzb();
        if (zzb != null) {
            return zzb.containsValue(obj);
        }
        for (int i = 0; i < this.zzg; i++) {
            if (zzcz.zza(obj, this.zzc[i])) {
                return true;
            }
        }
        return false;
    }

    @Override // java.util.AbstractMap, java.util.Map
    public final Set<Map.Entry<K, V>> entrySet() {
        Set<Map.Entry<K, V>> set = this.zzi;
        if (set == null) {
            C6626c c6626c = new C6626c(this);
            this.zzi = c6626c;
            return c6626c;
        }
        return set;
    }

    @Override // java.util.AbstractMap, java.util.Map
    public final V get(@NullableDecl Object obj) {
        Map<K, V> zzb = zzb();
        if (zzb != null) {
            return zzb.get(obj);
        }
        int zza = zza(obj);
        if (zza == -1) {
            return null;
        }
        return (V) this.zzc[zza];
    }

    @Override // java.util.AbstractMap, java.util.Map
    public final boolean isEmpty() {
        if (size() == 0) {
            return true;
        }
        return false;
    }

    @Override // java.util.AbstractMap, java.util.Map
    public final Set<K> keySet() {
        Set<K> set = this.zzh;
        if (set == null) {
            C6628e c6628e = new C6628e(this);
            this.zzh = c6628e;
            return c6628e;
        }
        return set;
    }

    /* JADX WARN: Multi-variable type inference failed */
    @Override // java.util.AbstractMap, java.util.Map
    @NullableDecl
    public final V put(@NullableDecl K k, @NullableDecl V v) {
        int min;
        int i;
        if (zza()) {
            zzde.zzb(zza(), "Arrays already allocated");
            int i2 = this.zzf;
            int max = Math.max(i2 + 1, 2);
            int highestOneBit = Integer.highestOneBit(max);
            if (max > ((int) (highestOneBit * 1.0d)) && (highestOneBit = highestOneBit << 1) <= 0) {
                i = 1073741824;
            } else {
                i = highestOneBit;
            }
            int max2 = Math.max(4, i);
            this.zze = AbstractC22841wk2.m725d(max2);
            zzb(max2 - 1);
            this.zza = new int[i2];
            this.zzb = new Object[i2];
            this.zzc = new Object[i2];
        }
        Map<K, V> zzb = zzb();
        if (zzb != null) {
            return zzb.put(k, v);
        }
        int[] iArr = this.zza;
        Object[] objArr = this.zzb;
        Object[] objArr2 = this.zzc;
        int i3 = this.zzg;
        int i4 = i3 + 1;
        int m68365b = AbstractC17613Fk2.m68365b(k);
        int zzi = zzi();
        int i5 = m68365b & zzi;
        int m727b = AbstractC22841wk2.m727b(this.zze, i5);
        if (m727b == 0) {
            if (i4 > zzi) {
                zzi = zza(zzi, AbstractC22841wk2.m723f(zzi), m68365b, i3);
            } else {
                AbstractC22841wk2.m724e(this.zze, i5, i4);
            }
        } else {
            int i6 = ~zzi;
            int i7 = m68365b & i6;
            int i8 = 0;
            while (true) {
                int i9 = m727b - 1;
                int i10 = iArr[i9];
                if ((i10 & i6) == i7 && zzcz.zza(k, objArr[i9])) {
                    V v2 = (V) objArr2[i9];
                    objArr2[i9] = v;
                    return v2;
                }
                int i11 = i10 & zzi;
                Object[] objArr3 = objArr;
                int i12 = i8 + 1;
                if (i11 == 0) {
                    if (i12 >= 9) {
                        LinkedHashMap linkedHashMap = new LinkedHashMap(zzi() + 1, 1.0f);
                        int zzd2 = zzd();
                        while (zzd2 >= 0) {
                            linkedHashMap.put(this.zzb[zzd2], this.zzc[zzd2]);
                            zzd2 = zza(zzd2);
                        }
                        this.zze = linkedHashMap;
                        this.zza = null;
                        this.zzb = null;
                        this.zzc = null;
                        zzc();
                        return (V) linkedHashMap.put(k, v);
                    } else if (i4 > zzi) {
                        zzi = zza(zzi, AbstractC22841wk2.m723f(zzi), m68365b, i3);
                    } else {
                        iArr[i9] = AbstractC22841wk2.m728a(i10, i4, zzi);
                    }
                } else {
                    i8 = i12;
                    m727b = i11;
                    objArr = objArr3;
                }
            }
        }
        int length = this.zza.length;
        if (i4 > length && (min = Math.min((int) LockFreeTaskQueueCore.MAX_CAPACITY_MASK, 1 | (Math.max(1, length >>> 1) + length))) != length) {
            this.zza = Arrays.copyOf(this.zza, min);
            this.zzb = Arrays.copyOf(this.zzb, min);
            this.zzc = Arrays.copyOf(this.zzc, min);
        }
        this.zza[i3] = AbstractC22841wk2.m728a(m68365b, 0, zzi);
        this.zzb[i3] = k;
        this.zzc[i3] = v;
        this.zzg = i4;
        zzc();
        return null;
    }

    @Override // java.util.AbstractMap, java.util.Map
    @NullableDecl
    public final V remove(@NullableDecl Object obj) {
        Map<K, V> zzb = zzb();
        if (zzb != null) {
            return zzb.remove(obj);
        }
        V v = (V) zzb(obj);
        if (v == zzd) {
            return null;
        }
        return v;
    }

    @Override // java.util.AbstractMap, java.util.Map
    public final int size() {
        Map<K, V> zzb = zzb();
        if (zzb != null) {
            return zzb.size();
        }
        return this.zzg;
    }

    @Override // java.util.AbstractMap, java.util.Map
    public final Collection<V> values() {
        Collection<V> collection = this.zzj;
        if (collection == null) {
            C6630g c6630g = new C6630g(this);
            this.zzj = c6630g;
            return c6630g;
        }
        return collection;
    }

    public final boolean zza() {
        return this.zze == null;
    }

    public final void zzc() {
        this.zzf += 32;
    }

    public final int zzd() {
        return isEmpty() ? -1 : 0;
    }

    public final Iterator<K> zze() {
        Map<K, V> zzb = zzb();
        if (zzb != null) {
            return zzb.keySet().iterator();
        }
        return new C6625b(this);
    }

    public final Iterator<Map.Entry<K, V>> zzf() {
        Map<K, V> zzb = zzb();
        if (zzb != null) {
            return zzb.entrySet().iterator();
        }
        return new C6624a(this);
    }

    public final Iterator<V> zzg() {
        Map<K, V> zzb = zzb();
        if (zzb != null) {
            return zzb.values().iterator();
        }
        return new C6627d(this);
    }

    private final int zza(int i, int i2, int i3, int i4) {
        Object m725d = AbstractC22841wk2.m725d(i2);
        int i5 = i2 - 1;
        if (i4 != 0) {
            AbstractC22841wk2.m724e(m725d, i3 & i5, i4 + 1);
        }
        Object obj = this.zze;
        int[] iArr = this.zza;
        for (int i6 = 0; i6 <= i; i6++) {
            int m727b = AbstractC22841wk2.m727b(obj, i6);
            while (m727b != 0) {
                int i7 = m727b - 1;
                int i8 = iArr[i7];
                int i9 = ((~i) & i8) | i6;
                int i10 = i9 & i5;
                int m727b2 = AbstractC22841wk2.m727b(m725d, i10);
                AbstractC22841wk2.m724e(m725d, i10, m727b);
                iArr[i7] = AbstractC22841wk2.m728a(i9, m727b2, i5);
                m727b = i8 & i;
            }
        }
        this.zze = m725d;
        zzb(i5);
        return i5;
    }

    public static /* synthetic */ int zzd(zzdp zzdpVar) {
        int i = zzdpVar.zzg;
        zzdpVar.zzg = i - 1;
        return i;
    }

    @NullableDecl
    public final Map<K, V> zzb() {
        Object obj = this.zze;
        if (obj instanceof Map) {
            return (Map) obj;
        }
        return null;
    }

    private final void zzb(int i) {
        this.zzf = AbstractC22841wk2.m728a(this.zzf, 32 - Integer.numberOfLeadingZeros(i), 31);
    }

    /* JADX INFO: Access modifiers changed from: private */
    @NullableDecl
    public final Object zzb(@NullableDecl Object obj) {
        if (zza()) {
            return zzd;
        }
        int zzi = zzi();
        int m726c = AbstractC22841wk2.m726c(obj, null, zzi, this.zze, this.zza, this.zzb, null);
        if (m726c == -1) {
            return zzd;
        }
        Object obj2 = this.zzc[m726c];
        zza(m726c, zzi);
        this.zzg--;
        zzc();
        return obj2;
    }

    /* JADX INFO: Access modifiers changed from: private */
    public final int zza(@NullableDecl Object obj) {
        if (zza()) {
            return -1;
        }
        int m68365b = AbstractC17613Fk2.m68365b(obj);
        int zzi = zzi();
        int m727b = AbstractC22841wk2.m727b(this.zze, m68365b & zzi);
        if (m727b == 0) {
            return -1;
        }
        int i = ~zzi;
        int i2 = m68365b & i;
        do {
            int i3 = m727b - 1;
            int i4 = this.zza[i3];
            if ((i4 & i) == i2 && zzcz.zza(obj, this.zzb[i3])) {
                return i3;
            }
            m727b = i4 & zzi;
        } while (m727b != 0);
        return -1;
    }

    public final void zza(int i, int i2) {
        int size = size();
        int i3 = size - 1;
        if (i < i3) {
            Object[] objArr = this.zzb;
            Object obj = objArr[i3];
            objArr[i] = obj;
            Object[] objArr2 = this.zzc;
            objArr2[i] = objArr2[i3];
            objArr[i3] = null;
            objArr2[i3] = null;
            int[] iArr = this.zza;
            iArr[i] = iArr[i3];
            iArr[i3] = 0;
            int m68365b = AbstractC17613Fk2.m68365b(obj) & i2;
            int m727b = AbstractC22841wk2.m727b(this.zze, m68365b);
            if (m727b == size) {
                AbstractC22841wk2.m724e(this.zze, m68365b, i + 1);
                return;
            }
            while (true) {
                int i4 = m727b - 1;
                int[] iArr2 = this.zza;
                int i5 = iArr2[i4];
                int i6 = i5 & i2;
                if (i6 == size) {
                    iArr2[i4] = AbstractC22841wk2.m728a(i5, i + 1, i2);
                    return;
                }
                m727b = i6;
            }
        } else {
            this.zzb[i] = null;
            this.zzc[i] = null;
            this.zza[i] = 0;
        }
    }

    public final int zza(int i) {
        int i2 = i + 1;
        if (i2 < this.zzg) {
            return i2;
        }
        return -1;
    }
}
