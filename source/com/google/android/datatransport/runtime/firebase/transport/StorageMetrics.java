package com.google.android.datatransport.runtime.firebase.transport;

import com.google.firebase.encoders.proto.Protobuf;

/* loaded from: classes3.dex */
public final class StorageMetrics {

    /* renamed from: c */
    public static final StorageMetrics f44136c = new Builder().build();

    /* renamed from: a */
    public final long f44137a;

    /* renamed from: b */
    public final long f44138b;

    /* loaded from: classes3.dex */
    public static final class Builder {

        /* renamed from: a */
        public long f44139a = 0;

        /* renamed from: b */
        public long f44140b = 0;

        public StorageMetrics build() {
            return new StorageMetrics(this.f44139a, this.f44140b);
        }

        public Builder setCurrentCacheSizeBytes(long j) {
            this.f44139a = j;
            return this;
        }

        public Builder setMaxCacheSizeBytes(long j) {
            this.f44140b = j;
            return this;
        }
    }

    public StorageMetrics(long j, long j2) {
        this.f44137a = j;
        this.f44138b = j2;
    }

    public static StorageMetrics getDefaultInstance() {
        return f44136c;
    }

    public static Builder newBuilder() {
        return new Builder();
    }

    @Protobuf(tag = 1)
    public long getCurrentCacheSizeBytes() {
        return this.f44137a;
    }

    @Protobuf(tag = 2)
    public long getMaxCacheSizeBytes() {
        return this.f44138b;
    }
}
