...
public class Draft_10 extends Draft {
    ...
    /* renamed from: a */
    public ByteBuffer f58440a;

    /* renamed from: b */
    public Framedata f58441b = null;

    /* renamed from: c */
    public final Random f58442c = new Random();
    ...
    /* renamed from: a */
    public final byte m33688a(Framedata.Opcode opcode) {
    ...
    public Draft.HandshakeState acceptHandshakeAsClient(ClientHandshake clientHandshake, ServerHandshake serverHandshake) throws InvalidHandshakeException {
        if (clientHandshake.hasFieldValue(HttpHeaders.SEC_WEBSOCKET_KEY) && serverHandshake.hasFieldValue(HttpHeaders.SEC_WEBSOCKET_ACCEPT)) {
            if (m33687b(clientHandshake.getFieldValue(HttpHeaders.SEC_WEBSOCKET_KEY)).equals(serverHandshake.getFieldValue(HttpHeaders.SEC_WEBSOCKET_ACCEPT))) {
    ...
    /* renamed from: b */
    public final String m33687b(String str) {
        try {
    ...
    /* renamed from: c */
    public final byte[] m33686c(long j, int i) {
    ...
    public ByteBuffer createBinaryFrame(Framedata framedata) {
        ...
        byte m33688a = m33688a(framedata.getOpcode());
        ...
        allocate.put((byte) (((byte) i4) | m33688a));
        byte[] m33686c = m33686c(payloadData.remaining(), i);
        if (i == 1) {
            byte b2 = m33686c[0];
            if (!z) {
                b = 0;
        ...
        if (z) {
            ...
            allocate2.putInt(this.f58442c.nextInt());
    ...
    /* renamed from: d */
    public final Framedata.Opcode m33685d(byte b) {
        if (b != 0) {
            if (b != 1) {
                if (b != 2) {
                    switch (b) {
    ...
    public HandshakeBuilder postProcessHandshakeResponseAsServer(ClientHandshake clientHandshake, ServerHandshakeBuilder serverHandshakeBuilder) throws InvalidHandshakeException {
        ...
        if (fieldValue != null) {
            serverHandshakeBuilder.put(HttpHeaders.SEC_WEBSOCKET_ACCEPT, m33687b(fieldValue));
    ...
    public List<Framedata> translateFrame(ByteBuffer byteBuffer) throws LimitExedeedException, InvalidDataException {
        ...
        if (this.f58440a != null) {
            try {
                ...
                int remaining2 = this.f58440a.remaining();
                if (remaining2 > remaining) {
                    this.f58440a.put(byteBuffer.array(), byteBuffer.position(), remaining);
                    byteBuffer.position(byteBuffer.position() + remaining);
                    return Collections.emptyList();
                }
                this.f58440a.put(byteBuffer.array(), byteBuffer.position(), remaining2);
                byteBuffer.position(byteBuffer.position() + remaining2);
                linkedList.add(translateSingleFrame((ByteBuffer) this.f58440a.duplicate().position(0)));
                this.f58440a = null;
            } catch (IncompleteException e) {
                this.f58440a.limit();
                ByteBuffer allocate = ByteBuffer.allocate(checkAlloc(e.getPreferedSize()));
                this.f58440a.rewind();
                allocate.put(this.f58440a);
                this.f58440a = allocate;
    ...
    public Framedata translateSingleFrame(ByteBuffer byteBuffer) throws IncompleteException, InvalidDataException {
        ...
        if (remaining >= 2) {
            ...
            if (b2 == 0) {
                ...
                Framedata.Opcode m33685d = m33685d((byte) (b & 15));
                if (!z && (m33685d == Framedata.Opcode.PING || m33685d == Framedata.Opcode.PONG || m33685d == Framedata.Opcode.CLOSING)) {
                    throw new InvalidFrameException("control frames may no be fragmented");
                }
                if (i3 < 0 || i3 > 125) {
                    if (m33685d != Framedata.Opcode.PING && m33685d != Framedata.Opcode.PONG && m33685d != Framedata.Opcode.CLOSING) {
                        if (i3 == 126) {
                            if (remaining >= 4) {
