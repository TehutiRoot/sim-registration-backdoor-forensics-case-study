...
public final class APIErrorExtensionsKt {
    ...
    public static final String getMessageFromResources(@NotNull APIError aPIError, @NotNull Resources res) {
        ...
        if (Intrinsics.areEqual(errorCode, APIErrorCode.AuthenticationFailure.INSTANCE)) {
            String string = res.getString(C5511R.C5518string.error_api_authentication_failure);
            ...
            String string2 = res.getString(C5511R.C5518string.error_api_service_not_found);
            Intrinsics.checkNotNullExpressionValue(string2, "res.getString(R.string.e…or_api_service_not_found)");
            return string2;
        } else {
            int i = C5511R.C5518string.error_required;
            String message = aPIError.getMessage();
            if (message != null) {
    ...
    public static final String getMessageWhenBadRequest(@NotNull APIError aPIError, @NotNull Resources res) {
        ...
        if (badRequestReason instanceof BadRequestReason.AmountIsGreaterThanValidAmount) {
            ...
            if (amountIsGreaterThanValidAmount.getValidAmount() != null && (currency2 = amountIsGreaterThanValidAmount.getCurrency()) != null && currency2.length() != 0) {
                message = res.getString(C5511R.C5518string.f40224x2568a915, new Amount(amountIsGreaterThanValidAmount.getValidAmount().longValue(), amountIsGreaterThanValidAmount.getCurrency()).toAmountString());
            } else {
                message = res.getString(C5511R.C5518string.f40225x4c3d1031);
            }
            ...
            if (amountIsLessThanValidAmount.getValidAmount() != null && (currency = amountIsLessThanValidAmount.getCurrency()) != null && currency.length() != 0) {
                message = res.getString(C5511R.C5518string.f40226x52b50dc, new Amount(amountIsLessThanValidAmount.getValidAmount().longValue(), amountIsLessThanValidAmount.getCurrency()).toAmountString());
            } else {
                message = res.getString(C5511R.C5518string.f40227x897f7f0a);
            }
        } else if (Intrinsics.areEqual(badRequestReason, BadRequestReason.InvalidCurrency.INSTANCE)) {
            message = res.getString(C5511R.C5518string.error_api_bad_request_invalid_currency);
        } else if (Intrinsics.areEqual(badRequestReason, BadRequestReason.EmptyName.INSTANCE)) {
            message = res.getString(C5511R.C5518string.error_api_bad_request_empty_name);
        } else if (badRequestReason instanceof BadRequestReason.NameIsTooLong) {
            message = res.getString(C5511R.C5518string.error_api_bad_request_name_is_too_long_with_valid_length, Integer.valueOf(((BadRequestReason.NameIsTooLong) badRequestReason).getMaximum()));
        } else if (Intrinsics.areEqual(badRequestReason, BadRequestReason.InvalidName.INSTANCE)) {
            message = res.getString(C5511R.C5518string.error_api_bad_request_invalid_name);
        } else if (Intrinsics.areEqual(badRequestReason, BadRequestReason.InvalidEmail.INSTANCE)) {
            message = res.getString(C5511R.C5518string.error_api_bad_request_invalid_email);
        } else if (Intrinsics.areEqual(badRequestReason, BadRequestReason.InvalidPhoneNumber.INSTANCE)) {
            message = res.getString(C5511R.C5518string.error_api_bad_request_invalid_phone_number);
        } else if (Intrinsics.areEqual(badRequestReason, BadRequestReason.SourceTypeNotSupported.INSTANCE)) {
            message = res.getString(C5511R.C5518string.error_api_bad_request_source_type_not_supported);
        } else if (Intrinsics.areEqual(badRequestReason, BadRequestReason.CurrencyNotSupported.INSTANCE)) {
            message = res.getString(C5511R.C5518string.error_api_bad_request_currency_not_supported);
        } else {
            message = aPIError.getMessage();
            if (message == null) {
                message = res.getString(C5511R.C5518string.error_unknown_without_reason);
    ...
    public static final String getMessageWhenInvalidCard(@NotNull APIError aPIError, @NotNull Resources res) {
        ...
        if (Intrinsics.areEqual(invalidCardReason, InvalidCardReason.InvalidCardNumber.INSTANCE)) {
            string = res.getString(C5511R.C5518string.error_api_invalid_card_invalid_card_number);
        } else if (Intrinsics.areEqual(invalidCardReason, InvalidCardReason.InvalidExpirationDate.INSTANCE)) {
            string = res.getString(C5511R.C5518string.error_api_invalid_card_invalid_expiration_date);
        } else if (Intrinsics.areEqual(invalidCardReason, InvalidCardReason.EmptyCardHolderName.INSTANCE)) {
            string = res.getString(C5511R.C5518string.error_api_invalid_card_empty_card_holder_name);
        } else if (Intrinsics.areEqual(invalidCardReason, InvalidCardReason.UnsupportedBrand.INSTANCE)) {
            string = res.getString(C5511R.C5518string.error_api_invalid_card_unsupported_brand);
        } else if (invalidCardReason instanceof InvalidCardReason.Unknown) {
            string = res.getString(C5511R.C5518string.error_required, ((InvalidCardReason.Unknown) invalidCardReason).getMessage());
        } else {
            int i = C5511R.C5518string.error_required;
            String message = aPIError.getMessage();
            if (message != null) {
