...
public class TargetUtils {
    /* renamed from: a */
    public static String m62664a(Collection collection) {
        ArrayList arrayList = new ArrayList();
        Iterator it = collection.iterator();
        while (it.hasNext()) {
            arrayList.add(getHumanReadableName(((Integer) it.next()).intValue()));
        }
        return "[" + ZQ1.m65238a(", ", arrayList) + "]";
    }

    public static void checkSupportedTargets(@NonNull Collection<Integer> collection, int i) {
        Preconditions.checkArgument(collection.contains(Integer.valueOf(i)), String.format(Locale.US, "Effects target %s is not in the supported list %s.", getHumanReadableName(i), m62664a(collection)));
    }
