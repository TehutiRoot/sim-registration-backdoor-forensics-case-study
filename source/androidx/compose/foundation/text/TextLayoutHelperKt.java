...
public final class TextLayoutHelperKt {
    ...
    public static final boolean m69802canReuse7_7YC6M(@NotNull TextLayoutResult canReuse, @NotNull AnnotatedString text, @NotNull TextStyle style, @NotNull List<AnnotatedString.Range<Placeholder>> placeholders, int i, boolean z, int i2, @NotNull Density density, @NotNull LayoutDirection layoutDirection, @NotNull FontFamily.Resolver fontFamilyResolver, long j) {
        ...
        if (canReuse.getMultiParagraph().getIntrinsics().getHasStaleResolvedFonts() || !Intrinsics.areEqual(layoutInput.getText(), text) || !layoutInput.getStyle().hasSameLayoutAffectingAttributes(style) || !Intrinsics.areEqual(layoutInput.getPlaceholders(), placeholders) || layoutInput.getMaxLines() != i || layoutInput.getSoftWrap() != z || !TextOverflow.m73587equalsimpl0(layoutInput.m73186getOverflowgIe3tQ8(), i2) || !Intrinsics.areEqual(layoutInput.getDensity(), density) || layoutInput.getLayoutDirection() != layoutDirection || !Intrinsics.areEqual(layoutInput.getFontFamilyResolver(), fontFamilyResolver) || Constraints.m73628getMinWidthimpl(j) != Constraints.m73628getMinWidthimpl(layoutInput.m73185getConstraintsmsEJaDk())) {
            return false;
        }
        if (!z && !TextOverflow.m73587equalsimpl0(i2, TextOverflow.Companion.m73595getEllipsisgIe3tQ8())) {
            return true;
        }
        if (Constraints.m73626getMaxWidthimpl(j) != Constraints.m73626getMaxWidthimpl(layoutInput.m73185getConstraintsmsEJaDk()) || Constraints.m73625getMaxHeightimpl(j) != Constraints.m73625getMaxHeightimpl(layoutInput.m73185getConstraintsmsEJaDk())) {
            return false;
        }
        return true;
    }
