...
public final class SnapFlingBehaviorKt$animateSnap$2 extends Lambda implements Function1<AnimationScope<Float, AnimationVector1D>, Unit> {
    ...
    public final void invoke2(@NotNull AnimationScope<Float, AnimationVector1D> animateTo) {
        ...
        m61440f = SnapFlingBehaviorKt.m61440f(animateTo.getValue().floatValue(), this.$cancelOffset);
        float f = m61440f - this.$consumedUpToNow.element;
        float scrollBy = this.$this_animateSnap.scrollBy(f);
        this.$onAnimationStep.invoke(Float.valueOf(scrollBy));
        if (Math.abs(f - scrollBy) > 0.5f || m61440f != animateTo.getValue().floatValue()) {
