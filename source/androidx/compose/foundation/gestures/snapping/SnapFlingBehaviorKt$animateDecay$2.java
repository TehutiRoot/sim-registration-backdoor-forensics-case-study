...
public final class SnapFlingBehaviorKt$animateDecay$2 extends Lambda implements Function1<AnimationScope<Float, AnimationVector1D>, Unit> {
    ...
    public final void invoke2(@NotNull AnimationScope<Float, AnimationVector1D> animateDecay) {
        ...
        if (Math.abs(animateDecay.getValue().floatValue()) >= Math.abs(this.$targetOffset)) {
            m61440f = SnapFlingBehaviorKt.m61440f(animateDecay.getValue().floatValue(), this.$targetOffset);
            SnapFlingBehaviorKt.m61444b(animateDecay, this.$this_animateDecay, this.$onAnimationStep, m61440f - this.$previousValue.element);
            animateDecay.cancelAnimation();
            this.$previousValue.element = m61440f;
            return;
        }
        SnapFlingBehaviorKt.m61444b(animateDecay, this.$this_animateDecay, this.$onAnimationStep, animateDecay.getValue().floatValue() - this.$previousValue.element);
