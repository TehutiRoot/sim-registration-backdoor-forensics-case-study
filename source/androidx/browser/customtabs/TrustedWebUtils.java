...
public class TrustedWebUtils {
    ...
    public static void launchBrowserSiteSettings(@NonNull Context context, @NonNull CustomTabsSession customTabsSession, @NonNull Uri uri) {
        ...
        intent.setPackage(customTabsSession.m64029g().getPackageName());
        intent.setData(uri);
        Bundle bundle = new Bundle();
        bundle.putBinder(CustomTabsIntent.EXTRA_SESSION, customTabsSession.m64030f());
        intent.putExtras(bundle);
        PendingIntent m64028h = customTabsSession.m64028h();
        if (m64028h != null) {
            intent.putExtra(CustomTabsIntent.EXTRA_SESSION_ID, m64028h);
        }
