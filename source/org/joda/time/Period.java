...
public final class Period extends BasePeriod implements ReadablePeriod, Serializable {
    ...
    public Days toStandardDays() {
        ...
        return Days.days(FieldUtils.safeToInt(FieldUtils.safeAdd(FieldUtils.safeAdd((((getMillis() + (getSeconds() * 1000)) + (getMinutes() * 60000)) + (getHours() * CoreConstants.MILLIS_IN_ONE_HOUR)) / 86400000, getDays()), getWeeks() * 7)));
    }

    public Duration toStandardDuration() {
        checkYearsAndMonths("Duration");
        return new Duration(getMillis() + (getSeconds() * 1000) + (getMinutes() * 60000) + (getHours() * CoreConstants.MILLIS_IN_ONE_HOUR) + (getDays() * 86400000) + (getWeeks() * CoreConstants.MILLIS_IN_ONE_WEEK));
    }

    public Hours toStandardHours() {
        checkYearsAndMonths("Hours");
        return Hours.hours(FieldUtils.safeToInt(FieldUtils.safeAdd(FieldUtils.safeAdd(FieldUtils.safeAdd(((getMillis() + (getSeconds() * 1000)) + (getMinutes() * 60000)) / CoreConstants.MILLIS_IN_ONE_HOUR, getHours()), getDays() * 24), getWeeks() * 168)));
    }
    ...
    public Weeks toStandardWeeks() {
        ...
        return Weeks.weeks(FieldUtils.safeToInt(getWeeks() + (((((getMillis() + (getSeconds() * 1000)) + (getMinutes() * 60000)) + (getHours() * CoreConstants.MILLIS_IN_ONE_HOUR)) + (getDays() * 86400000)) / CoreConstants.MILLIS_IN_ONE_WEEK)));
    }
    ...
    public Period withPeriodType(PeriodType periodType) {
        ...
        return new Period(this, periodType2);
    }
    ...
    public Period normalizedStandard(PeriodType periodType) {
        PeriodType periodType2 = DateTimeUtils.getPeriodType(periodType);
        Period period = new Period(getMillis() + (getSeconds() * 1000) + (getMinutes() * 60000) + (getHours() * CoreConstants.MILLIS_IN_ONE_HOUR) + (getDays() * 86400000) + (getWeeks() * CoreConstants.MILLIS_IN_ONE_WEEK), periodType2, ISOChronology.getInstanceUTC());
        int years = getYears();
