...
public class McElieceKobaraImaiCipher implements MessageEncryptor {
    ...
    private void initCipherDecrypt(McElieceCCA2PrivateKeyParameters mcElieceCCA2PrivateKeyParameters) {
        ...
        this.f75539n = mcElieceCCA2PrivateKeyParameters.getN();
        this.f75538k = mcElieceCCA2PrivateKeyParameters.getK();
        this.f75541t = mcElieceCCA2PrivateKeyParameters.getT();
    }

    private void initCipherEncrypt(McElieceCCA2PublicKeyParameters mcElieceCCA2PublicKeyParameters) {
        this.messDigest = Utils.getDigest(mcElieceCCA2PublicKeyParameters.getDigest());
        this.f75539n = mcElieceCCA2PublicKeyParameters.getN();
        this.f75538k = mcElieceCCA2PublicKeyParameters.getK();
        this.f75541t = mcElieceCCA2PublicKeyParameters.getT();
    }
    ...
    public void init(boolean z, CipherParameters cipherParameters) {
        ...
        if (!z) {
            ...
            this.f75540sr = CryptoServicesRegistrar.getSecureRandom();
            McElieceCCA2PublicKeyParameters mcElieceCCA2PublicKeyParameters = (McElieceCCA2PublicKeyParameters) cipherParameters;
            this.key = mcElieceCCA2PublicKeyParameters;
            initCipherEncrypt(mcElieceCCA2PublicKeyParameters);
        } else {
            ParametersWithRandom parametersWithRandom = (ParametersWithRandom) cipherParameters;
            this.f75540sr = parametersWithRandom.getRandom();
    ...
    public byte[] messageDecrypt(byte[] bArr) throws InvalidCipherTextException {
        ...
        if (bArr.length >= i) {
            ...
            GF2Vector[] decryptionPrimitive = McElieceCCA2Primitives.decryptionPrimitive((McElieceCCA2PrivateKeyParameters) this.key, GF2Vector.OS2VP(this.f75539n, bArr));
            byte[] encoded = decryptionPrimitive[0].getEncoded();
            GF2Vector gF2Vector = decryptionPrimitive[1];
            if (encoded.length > i2) {
                encoded = ByteUtils.subArray(encoded, 0, i2);
            }
            byte[] concatenate = ByteUtils.concatenate(ByteUtils.concatenate(bArr2, Conversions.decode(this.f75539n, this.f75541t, gF2Vector)), encoded);
    ...
    public byte[] messageEncrypt(byte[] bArr) {
        if (this.forEncryption) {
            int digestSize = this.messDigest.getDigestSize();
            int i = this.f75538k >> 3;
            int bitLength = (IntegerFunctions.binomial(this.f75539n, this.f75541t).bitLength() - 1) >> 3;
            ...
            byte[] encoded = McElieceCCA2Primitives.encryptionPrimitive((McElieceCCA2PublicKeyParameters) this.key, GF2Vector.OS2VP(this.f75538k, bArr9), Conversions.encode(this.f75539n, this.f75541t, bArr8)).getEncoded();
