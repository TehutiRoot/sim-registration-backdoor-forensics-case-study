...
public class McElieceCipher implements MessageEncryptor {
    ...
    private GF2Vector computeMessageRepresentative(byte[] bArr) {
        byte[] bArr2 = new byte[this.maxPlainTextSize + ((this.f75527k & 7) != 0 ? 1 : 0)];
        System.arraycopy(bArr, 0, bArr2, 0, bArr.length);
        bArr2[bArr.length] = 1;
        return GF2Vector.OS2VP(this.f75527k, bArr2);
    }

    private void initCipherDecrypt(McEliecePrivateKeyParameters mcEliecePrivateKeyParameters) {
        this.f75528n = mcEliecePrivateKeyParameters.getN();
        int k = mcEliecePrivateKeyParameters.getK();
        this.f75527k = k;
        this.maxPlainTextSize = k >> 3;
        this.cipherTextSize = this.f75528n >> 3;
    }

    private void initCipherEncrypt(McEliecePublicKeyParameters mcEliecePublicKeyParameters) {
        this.f75528n = mcEliecePublicKeyParameters.getN();
        this.f75527k = mcEliecePublicKeyParameters.getK();
        this.f75530t = mcEliecePublicKeyParameters.getT();
        this.cipherTextSize = this.f75528n >> 3;
        this.maxPlainTextSize = this.f75527k >> 3;
    }
    ...
    public void init(boolean z, CipherParameters cipherParameters) {
        ...
        if (!z) {
            ...
            this.f75529sr = CryptoServicesRegistrar.getSecureRandom();
            ...
            this.f75529sr = parametersWithRandom.getRandom();
    ...
    public byte[] messageDecrypt(byte[] bArr) throws InvalidCipherTextException {
        ...
        GF2Vector OS2VP = GF2Vector.OS2VP(this.f75528n, bArr);
        ...
        return computeMessage((GF2Vector) sInv.leftMultiply(((GF2Vector) ((GF2Vector) gF2Vector.add(syndromeDecode)).multiply(p1)).extractRightVector(this.f75527k)));
    }
    ...
    public byte[] messageEncrypt(byte[] bArr) {
        if (this.forEncryption) {
            ...
            return ((GF2Vector) ((McEliecePublicKeyParameters) this.key).getG().leftMultiply(computeMessageRepresentative).add(new GF2Vector(this.f75528n, this.f75530t, this.f75529sr))).getEncoded();
        }
        throw new IllegalStateException("cipher initialised for decryption");
    }
}
