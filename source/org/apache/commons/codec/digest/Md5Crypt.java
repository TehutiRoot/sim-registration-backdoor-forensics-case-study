package org.apache.commons.codec.digest;

import java.nio.charset.Charset;
import java.nio.charset.StandardCharsets;
import java.security.MessageDigest;
import java.security.SecureRandom;
import java.util.Arrays;
import java.util.Objects;
import java.util.Random;
import java.util.regex.Matcher;
import java.util.regex.Pattern;

/* loaded from: classes6.dex */
public class Md5Crypt {
    public static String apr1Crypt(byte[] bArr) {
        return apr1Crypt(bArr, "$apr1$" + AbstractC1293S9.m66365b(8));
    }

    public static String md5Crypt(byte[] bArr) {
        return md5Crypt(bArr, "$1$" + AbstractC1293S9.m66365b(8));
    }

    public static String apr1Crypt(byte[] bArr, Random random) {
        return apr1Crypt(bArr, "$apr1$" + AbstractC1293S9.m66364c(8, random));
    }

    public static String md5Crypt(byte[] bArr, Random random) {
        return md5Crypt(bArr, "$1$" + AbstractC1293S9.m66364c(8, random));
    }

    public static String apr1Crypt(byte[] bArr, String str) {
        if (str != null && !str.startsWith("$apr1$")) {
            str = "$apr1$" + str;
        }
        return md5Crypt(bArr, str, "$apr1$");
    }

    public static String md5Crypt(byte[] bArr, String str) {
        return md5Crypt(bArr, str, "$1$");
    }

    public static String md5Crypt(byte[] bArr, String str, String str2) {
        return md5Crypt(bArr, str, str2, new SecureRandom());
    }

    public static String md5Crypt(byte[] bArr, String str, String str2, Random random) {
        String group;
        int length = bArr.length;
        if (str == null) {
            group = AbstractC1293S9.m66364c(8, random);
        } else {
            Objects.requireNonNull(str2, "prefix");
            if (str2.length() >= 3) {
                if (str2.charAt(0) != '$' && str2.charAt(str2.length() - 1) != '$') {
                    throw new IllegalArgumentException("Invalid prefix value: " + str2);
                }
                Matcher matcher = Pattern.compile("^" + str2.replace("$", "\\$") + "([\\.\\/a-zA-Z0-9]{1,8}).*").matcher(str);
                if (matcher.find()) {
                    group = matcher.group(1);
                } else {
                    throw new IllegalArgumentException("Invalid salt value: " + str);
                }
            } else {
                throw new IllegalArgumentException("Invalid prefix value: " + str2);
            }
        }
        Charset charset = StandardCharsets.UTF_8;
        byte[] bytes = group.getBytes(charset);
        MessageDigest md5Digest = DigestUtils.getMd5Digest();
        md5Digest.update(bArr);
        md5Digest.update(str2.getBytes(charset));
        md5Digest.update(bytes);
        MessageDigest md5Digest2 = DigestUtils.getMd5Digest();
        md5Digest2.update(bArr);
        md5Digest2.update(bytes);
        md5Digest2.update(bArr);
        byte[] digest = md5Digest2.digest();
        for (int i = length; i > 0; i -= 16) {
            md5Digest.update(digest, 0, Math.min(i, 16));
        }
        Arrays.fill(digest, (byte) 0);
        while (length > 0) {
            if ((length & 1) == 1) {
                md5Digest.update(digest[0]);
            } else {
                md5Digest.update(bArr[0]);
            }
            length >>= 1;
        }
        StringBuilder sb = new StringBuilder(str2 + group + "$");
        byte[] digest2 = md5Digest.digest();
        for (int i2 = 0; i2 < 1000; i2++) {
            md5Digest2 = DigestUtils.getMd5Digest();
            int i3 = i2 & 1;
            if (i3 != 0) {
                md5Digest2.update(bArr);
            } else {
                md5Digest2.update(digest2, 0, 16);
            }
            if (i2 % 3 != 0) {
                md5Digest2.update(bytes);
            }
            if (i2 % 7 != 0) {
                md5Digest2.update(bArr);
            }
            if (i3 != 0) {
                md5Digest2.update(digest2, 0, 16);
            } else {
                md5Digest2.update(bArr);
            }
            digest2 = md5Digest2.digest();
        }
        AbstractC1293S9.m66366a(digest2[0], digest2[6], digest2[12], 4, sb);
        AbstractC1293S9.m66366a(digest2[1], digest2[7], digest2[13], 4, sb);
        AbstractC1293S9.m66366a(digest2[2], digest2[8], digest2[14], 4, sb);
        AbstractC1293S9.m66366a(digest2[3], digest2[9], digest2[15], 4, sb);
        AbstractC1293S9.m66366a(digest2[4], digest2[10], digest2[5], 4, sb);
        AbstractC1293S9.m66366a((byte) 0, (byte) 0, digest2[11], 2, sb);
        md5Digest.reset();
        md5Digest2.reset();
        Arrays.fill(bArr, (byte) 0);
        Arrays.fill(bytes, (byte) 0);
        Arrays.fill(digest2, (byte) 0);
        return sb.toString();
    }

    public static String apr1Crypt(String str) {
        return apr1Crypt(str.getBytes(StandardCharsets.UTF_8));
    }

    public static String apr1Crypt(String str, String str2) {
        return apr1Crypt(str.getBytes(StandardCharsets.UTF_8), str2);
    }
}
