...
public class RetryExec implements ClientExecChain {
    ...
    /* renamed from: a */
    public final Log f74427a = LogFactory.getLog(getClass());

    /* renamed from: b */
    public final ClientExecChain f74428b;

    /* renamed from: c */
    public final HttpRequestRetryHandler f74429c;
    ...
    public RetryExec(ClientExecChain clientExecChain, HttpRequestRetryHandler httpRequestRetryHandler) {
        ...
        this.f74428b = clientExecChain;
        this.f74429c = httpRequestRetryHandler;
    }
    ...
    public CloseableHttpResponse execute(HttpRoute httpRoute, HttpRequestWrapper httpRequestWrapper, HttpClientContext httpClientContext, HttpExecutionAware httpExecutionAware) throws IOException, HttpException {
        ...
        while (true) {
            try {
                return this.f74428b.execute(httpRoute, httpRequestWrapper, httpClientContext, httpExecutionAware);
            } catch (IOException e) {
                if (httpExecutionAware != null && httpExecutionAware.isAborted()) {
                    this.f74427a.debug("Request has been aborted");
                    throw e;
                } else if (this.f74429c.retryRequest(e, i, httpClientContext)) {
                    if (this.f74427a.isInfoEnabled()) {
                        Log log = this.f74427a;
                        log.info("I/O exception (" + e.getClass().getName() + ") caught when processing request to " + httpRoute + ": " + e.getMessage());
                    }
                    if (this.f74427a.isDebugEnabled()) {
                        this.f74427a.debug(e.getMessage(), e);
                    }
                    if (C19955fx1.m31247d(httpRequestWrapper)) {
                        httpRequestWrapper.setHeaders(allHeaders);
                        if (this.f74427a.isInfoEnabled()) {
                            Log log2 = this.f74427a;
                            log2.info("Retrying request to " + httpRoute);
                        }
                        i++;
                    } else {
                        this.f74427a.debug("Cannot retry non-repeatable request");
