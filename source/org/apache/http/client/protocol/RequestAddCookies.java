...
public class RequestAddCookies implements HttpRequestInterceptor {
    ...
    /* renamed from: a */
    public final Log f73722a = LogFactory.getLog(getClass());
    ...
    public void process(HttpRequest httpRequest, HttpContext httpContext) throws HttpException, IOException {
        ...
        if (cookieStore == null) {
            this.f73722a.debug("Cookie store not specified in HTTP context");
            return;
        }
        Lookup<CookieSpecProvider> cookieSpecRegistry = adapt.getCookieSpecRegistry();
        if (cookieSpecRegistry == null) {
            this.f73722a.debug("CookieSpec registry not specified in HTTP context");
            return;
        }
        HttpHost targetHost = adapt.getTargetHost();
        if (targetHost == null) {
            this.f73722a.debug("Target host not set in the context");
            return;
        }
        RouteInfo httpRoute = adapt.getHttpRoute();
        if (httpRoute == null) {
            this.f73722a.debug("Connection route not set in the context");
            return;
        }
        ...
        if (this.f73722a.isDebugEnabled()) {
            this.f73722a.debug("CookieSpec selected: " + cookieSpec);
        }
        String str = null;
        ...
        if (lookup == null) {
            if (this.f73722a.isDebugEnabled()) {
                this.f73722a.debug("Unsupported cookie policy: " + cookieSpec);
                return;
            }
            return;
        }
        ...
        for (Cookie cookie : cookies) {
            if (!cookie.isExpired(date)) {
                if (create.match(cookie, cookieOrigin)) {
                    if (this.f73722a.isDebugEnabled()) {
                        this.f73722a.debug("Cookie " + cookie + " match " + cookieOrigin);
                    }
                    arrayList.add(cookie);
                }
            } else {
                if (this.f73722a.isDebugEnabled()) {
                    this.f73722a.debug("Cookie " + cookie + " expired");
                }
                z = true;
            }
        }
