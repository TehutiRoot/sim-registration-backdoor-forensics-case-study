...
public final class CharArrayBuilder extends PrimitiveArrayBuilder<char[]> {
    ...
    public CharArrayBuilder(@NotNull char[] bufferWithData) {
        ...
        this.f71175a = bufferWithData;
        this.f71176b = bufferWithData.length;
        ensureCapacity$kotlinx_serialization_core(10);
    }

    public final void append$kotlinx_serialization_core(char c) {
        PrimitiveArrayBuilder.ensureCapacity$kotlinx_serialization_core$default(this, 0, 1, null);
        char[] cArr = this.f71175a;
        int position$kotlinx_serialization_core = getPosition$kotlinx_serialization_core();
        this.f71176b = position$kotlinx_serialization_core + 1;
    ...
    public void ensureCapacity$kotlinx_serialization_core(int i) {
        char[] cArr = this.f71175a;
        if (cArr.length < i) {
            char[] copyOf = Arrays.copyOf(cArr, AbstractC11719c.coerceAtLeast(i, cArr.length * 2));
    ...
    public char[] build$kotlinx_serialization_core() {
        char[] copyOf = Arrays.copyOf(this.f71175a, getPosition$kotlinx_serialization_core());
        Intrinsics.checkNotNullExpressionValue(copyOf, "copyOf(...)");
        return copyOf;
    }
}
