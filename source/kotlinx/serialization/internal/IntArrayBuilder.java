...
public final class IntArrayBuilder extends PrimitiveArrayBuilder<int[]> {
    ...
    public void ensureCapacity$kotlinx_serialization_core(int i) {
        int[] iArr = this.f71205a;
        if (iArr.length < i) {
            int[] copyOf = Arrays.copyOf(iArr, AbstractC11719c.coerceAtLeast(i, iArr.length * 2));
    ...
    public int[] build$kotlinx_serialization_core() {
        int[] copyOf = Arrays.copyOf(this.f71205a, getPosition$kotlinx_serialization_core());
        Intrinsics.checkNotNullExpressionValue(copyOf, "copyOf(...)");
        return copyOf;
    }
}
