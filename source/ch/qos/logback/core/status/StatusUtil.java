...
public class StatusUtil {
    ...
    /* renamed from: a */
    public StatusManager f40134a;

    public StatusUtil(Context context) {
        this.f40134a = context.getStatusManager();
    }
    ...
    public void addStatus(Status status) {
        StatusManager statusManager = this.f40134a;
    ...
    public boolean containsException(Class<?> cls) {
        for (Status status : this.f40134a.getCopyOfStatusList()) {
    ...
    public int getHighestLevel(long j) {
        int i = 0;
        for (Status status : filterStatusListByTimeThreshold(this.f40134a.getCopyOfStatusList(), j)) {
    ...
    public long timeOfLastReset() {
        List<Status> copyOfStatusList = this.f40134a.getCopyOfStatusList();
    ...
    public StatusUtil(StatusManager statusManager) {
        this.f40134a = statusManager;
    }

    public boolean containsMatch(long j, int i, String str) {
        List<Status> filterStatusListByTimeThreshold = filterStatusListByTimeThreshold(this.f40134a.getCopyOfStatusList(), j);
        Pattern compile = Pattern.compile(str);
