...
public final class CompletableConcatIterable extends Completable {
    ...
    public static final class ConcatInnerObserver extends AtomicInteger implements CompletableObserver {
        ...
        /* renamed from: sd */
        final SequentialDisposable f63904sd = new SequentialDisposable();
        ...
        public void next() {
            if (this.f63904sd.isDisposed() || getAndIncrement() != 0) {
                return;
            }
            Iterator<? extends CompletableSource> it = this.sources;
            while (!this.f63904sd.isDisposed()) {
                try {
                    if (!it.hasNext()) {
        ...
        public void onSubscribe(Disposable disposable) {
            this.f63904sd.replace(disposable);
        }
    }

    public CompletableConcatIterable(Iterable<? extends CompletableSource> iterable) {
        this.f63903a = iterable;
    }

    @Override // io.reactivex.Completable
    public void subscribeActual(CompletableObserver completableObserver) {
        try {
            ConcatInnerObserver concatInnerObserver = new ConcatInnerObserver(completableObserver, (Iterator) ObjectHelper.requireNonNull(this.f63903a.iterator(), "The iterator returned is null"));
            completableObserver.onSubscribe(concatInnerObserver.f63904sd);
